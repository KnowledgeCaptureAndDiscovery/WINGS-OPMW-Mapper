...
public class ModelUtils {
    ...
    public static OntModel loadModel(String path, String lang) throws IllegalArgumentException {
        OntModel m = ModelFactory.createOntologyModel();
        if (path.startsWith("http://")) {
            m.read(path);
            return m;
    ...
    static DatasetGraph loadDatasetGraph(String absolutePath) {
        /**
         * Load a nquads file in a dataset
         */
        try {
            DatasetGraph dataset = RDFDataMgr.loadDatasetGraph(absolutePath, Lang.NQUADS);
            return dataset;
        } catch (Exception e) {
