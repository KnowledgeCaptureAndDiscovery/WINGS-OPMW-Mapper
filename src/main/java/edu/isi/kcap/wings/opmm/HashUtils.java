...
/**
 * Class that defines a set of methods for encoding components, files,
 * templates, etc.
...
public class HashUtils {
  ...
  /**
   * Function to create an md5 signature for components. It uses: inputs names,
   * output names and component name.
   * For simplicity, changes in hardware deps, software deps and isConcrete do not
   * constitute a new component version.
   * Changes in code do not constitute a new component version. Changes in the
   * input types do not constitute a new version.
   *
   ...
   * @param wingsTaxonomy      WINGS component catalog. Necessary to access the
   *                           component I/O.
   * @return
   */
  public static String createMD5ForTemplate(Individual templateInstance, OntModel wingsTemplateModel,
    ...
    StmtIterator propertiesIt = templateInstance
        .listProperties(wingsTemplateModel.getProperty(Constants.WINGS_PROP_HAS_LINK));
    while (propertiesIt.hasNext()) {
      Statement s = propertiesIt.next();
      connectionsAndComponents.add(s.getObject().asResource().getURI());
    }
    // for retrieving components, a query is easier than asking
    // nodes/hasComponent/hasComponentBinding
  ...
  public static void main(String[] arges) {
    ...
    OntModel m = ModelUtils.loadModel(
        "C:\\Users\\dgarijo\\Dropbox (OEG-UPM)\\NetBeansProjects\\WingsToOPMWMapper\\NEW_TEST\\concreteWorkflow\\MpileupVariantCallerV1.owl");
    OntModel taxonomy = ModelUtils.loadModel(
        "http://www.wings-workflows.org/wings-omics-portal/export/users/ravali/genomics/components/library.owl");
    Individual templateInstance = m.getIndividual(
        "http://www.wings-workflows.org/wings-omics-portal/export/users/ravali/genomics/workflows/MpileupVariantCaller.owl#MpileupVariantCaller");
