...
public class Mapper {
        ...
        /**
         * Query the local Wings repository. These functions are here to facilitate
         * coding
         *
         ...
         * @param modeFile. syntax of the files to load: "RDF/XML", "N-TRIPLE", "TURTLE"
         *                  (or "TTL") and "N3"
        ...
        /**
         * Method to load the domain specific taxonomy. Used to determine the node
         * types.
         *
        ...
        private void loadTaxonomy(OntModel m) throws Exception {
                ...
                // System.out.println("Exporting the componentCatalog to a separate file
                // outside");
         ...
         * @param componentCatalogPath   path for reading exporting the generated
         *                               taxonomy.
         * @param exportMode
         * @param componentDirectory
         * @param domainName             name of the domain in which the workflow is
         *                               published
         * @return Template URI assigned to identify the template
         * @throws IOException
         */
        // public String transformWINGSElaboratedTemplateToOPMW(String template,String
        // mode, String outFile){
        public String transformWINGSElaboratedTemplateToOPMW(String wingsInputTemplatePath, String inputMode,
                        String rdfOutputFile, String componentCatalogPath, String exportMode, String componentDirectory,
                        String domainName) throws IOException {
                NEW_TAXONOMY_CLASS = Constants.CATALOG_URI + domainName + "#";
                NEW_TAXONOMY_CLASS_2 = Constants.CATALOG_URI + domainName + "/";
                ...
                if (r.hasNext()) {// there should be just one local name per template
                        ...
                        ModelUtils.addIndividual(OPMWModel, newTemplateName, Constants.OPMW_WORKFLOW_TEMPLATE,
                                        newTemplateName);

                        OntClass cParam = OPMWModel.createClass(Constants.P_PLAN_PLAN);
                        cParam.createIndividual(
                                        Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_WORKFLOW_TEMPLATE + "/"
                                                        + EncodingUtils.encode(newTemplateName));
                ...
                while (r.hasNext()) {
                        ...
                        if (contrib != null) {
                                ...
                                ModelUtils.addIndividual(OPMWModel, contrib.getString(), Constants.OPM_AGENT,
                                                "Agent " + contrib.getString());
                                ModelUtils.addProperty(OPMWModel,
                                                Constants.CONCEPT_WORKFLOW_TEMPLATE + "/" + newTemplateName,
                                                Constants.CONCEPT_AGENT + "/" + contrib.getString(),
                                                Constants.PROP_HAS_CONTRIBUTOR);

                                // prov-o interoperability
                                String agEncoded = EncodingUtils
                                                .encode(Constants.CONCEPT_AGENT + "/" + contrib.getString());
                ...
                ModelUtils.addDataProperty(OPMWModel, Constants.CONCEPT_WORKFLOW_TEMPLATE + "/" + newTemplateName,
                                ...
                                Constants.DC_LICENSE, XSDDatatype.XSDanyURI);
                ...
                ModelUtils.addDataProperty(OPMWModel, Constants.CONCEPT_WORKFLOW_TEMPLATE + "/" + newTemplateName,
                                xmlDateTime,
                                Constants.DC_ISSUED_TIME, XSDDatatype.XSDdateTime);

                // retrieval of the Components (nodes, with their components and if they are
                // concrete or not)
                ...
                // CREATING 2 HASHSETS FOR MAINTAINING THE CORRECT ORDER IN WHICH THE COMPONENTS
                // ENTER IN THE CASES OF ABSTRACT COMPS AND CONCRETE COMPS
                while (r.hasNext()) {
                        QuerySolution qs = r.next();
                        ...
                        String componentCatalogQueryforSubclassCheck = Queries
                                        .componentCatalogQueryforSubclassCheckfinal(className);
                        ...
                        while (rnew1.hasNext()) {
                                ...
                                if (x != null) {
                                        ...
                                        if (nodenew.getLocalName().equals(className) && concr != null) {
                                                ...
                                                System.out.println("y is " + y.getLocalName() + " n is "
                                                                + nodenew.getLocalName() + " x is "
                                                                + x.getLocalName() + " concreteness is " + concr);
                                                deciderpoint = 1;
                                                break;
                                        }
                        ...
                        if (concreteness != null && concreteness.equals("false")) {
                                if (!AbstractSuperClass.getLocalName().equals("Component"))
                                        specialCaseforFolders.add(
                                                        concrComponent.getLocalName().substring(0,
                                                                        concrComponent.getLocalName().length() - 5));
                                hs2Abs.add(concrComponent.getLocalName().substring(0,
                                                concrComponent.getLocalName().length() - 5));
                        }
                }
                ...
                while (r.hasNext()) {
                        ...
                        if (concreteness != null && concreteness.equals("true"))// &&
                                                                                // !AbstractSuperClass.getLocalName().equals("Component")
                        {

                                hs1Concr.add(concrComponent.getLocalName().substring(0,
                                                concrComponent.getLocalName().length() - 5));
                                hs2Abs.remove(
                                                AbstractSuperClass.getLocalName().substring(0,
                                                                AbstractSuperClass.getLocalName().length() - 5));
                        }
                ...
                // created this hashset since some of the components repeat and we don't want to
                // continuously find them in the
                ...
                while (r.hasNext()) {
                        ...
                        System.out.println(res + " Node has component " + comp + " of type: " + typeComp);// + " which
                                                                                                          // is
                                                                                                          // concrete:
                                                                                                          // "+isConcrete.getBoolean()
                        if (res != null) {
                        ...
                        if (!hs1Concr.contains(className.substring(0, className.length() - 5))
                                        && !hs2Abs.contains(className.substring(0, className.length() - 5))
                                        && !hs1Concr.contains(className)
                                        && !hs2Abs.contains(className)) {
                        ...
                        // String
                        // temp444=temp333.substring(temp333.lastIndexOf("/")+1,temp333.length());
                        ...
                        ModelUtils.addIndividual(OPMWModel, newTemplateName_ + res.getLocalName(),
                                        Constants.OPMW_WORKFLOW_TEMPLATE_PROCESS,
                                        "Workflow template process " + res.getLocalName());
                        // p-plan interop
                        OntClass cStep = OPMWModel.createClass(Constants.P_PLAN_STEP);
                        cStep.createIndividual(Constants.PREFIX_EXPORT_RESOURCE
                                        + Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/"
                                        + EncodingUtils.encode(newTemplateName_ + res.getLocalName()));

                        // Have to start querying the component catalog for checking the number of
                        // inputs and outputs:
                        ...
                        if (!hsforComps.contains(className)) {
                                ...
                                System.out.println("ULTIMATE WORTH IT SUPER CLASS IS "
                                                + AbstractSuperClass.getLocalName());

                                if (deciderPoint == 1) {

                                        System.out.println(
                                                        "THIS MEANS THE BASIC CONDITIONS ARE MET THAT THE COMPONENT-CATALOG HAS EXPORTED THE COMPONENT---------");
                                        ...
                                        String componentCatalogQueryforInputsandOutputsComponent = Queries
                                                        .componentCatalogQueryforInputsandOutputs(PREFIX_COMP_CATALOG,
                                                                        className);
                                        ResultSet rnew2 = null;
                                        rnew2 = queryComponentCatalog(
                                                        componentCatalogQueryforInputsandOutputsComponent);
                                        ...
                                        while (rnew2.hasNext()) {
                                                ...
                                                if (nodenew12.getLocalName().equals(
                                         ...
                                         * 4. IF FOUND BUT HAS DIFFERENT NUMBER OF INPUTS AND OUTPUTS THEN CREATE A NEW
                                         * VERSION
                                         * AND LINK EXISTING LAST MODIFIED VERSION TO THE NEW ONE
                                         *
                                         */

                                        if (hs2Abs.contains(className)
                                                        || hs2Abs.contains(
                                                                        className.substring(0, className.length() - 5))
                                                                        && nodenew11 != null) {
                                                boolean specialFolder = false;
                                                if (specialCaseforFolders.contains(className)
                                                                || specialCaseforFolders.contains(className.substring(0,
                                                                                className.length() - 5))) {
                                                ...
                                                String taxonomyModelforAbstractComponent = Queries
                                                                .TaxonomyExportQueryforSubclassCheckfinal();
                                                ResultSet r2new = null;
                                                r2new = queryLocalTaxonomyModelRepository(
                                                                taxonomyModelforAbstractComponent);
                                                ...
                                                while (r2new.hasNext()) {
                                                        ...
                                                        Resource nodenew = qsnew.getResource("?n");
                                                        Resource x = qsnew.getResource("?x");
                                                        boolean isComponentConcrete = qsnew.getLiteral("?isConcrete")
                                                                        .getBoolean();
                                                        System.out.println(
                                                                        "What is nodenew? " + nodenew.getLocalName());
                                                        ...
                                                        if (!isComponentConcrete) {
                                                                ...
                                                                if (nodenew.getLocalName().toLowerCase()
                                                                                .contains(nodenew11.getLocalName()
                                                                                                .toLowerCase())) {
                                                                        // this means the name exists
                                                                        // next step is to check the inputs and outputs
                                                                        // based on that className
                                                                        // saving all such cases in a HashSet
                                                                        if (!namesOfAbstractCompswithSameNames.contains(
                                                                                        nodenew.getLocalName())) {
                                                                                namesOfAbstractCompswithSameNames.add(
                                                                                                nodenew.getLocalName());
                                                                        }
                                                ...
                                                if (namesOfAbstractCompswithSameNames.isEmpty()) {
                                                        // this means that there are no similarities anywhere and we
                                                        // need to export it
                                                        System.out.println("NOW UR INSIDE EXPORTING A FRESH ONE");
                                                        // EXPORTING IT NOW:
                                                        System.out.println("ULTIMATE POINT ::::CHECK::::"
                                                                        + nodenew11.getLocalName());
                                                        ...
                                                        System.out.println(
                                                                        "EXPORTED THE BASIC ABSTRACT COMPONENT BY HERE");
                                                        // export of component ends

                                                        // start exporting the inputs and outputs for the abstract
                                                        // component and
                                                        // canonical instance

                                                        // extracting the actual inputs,outputs and other factors for
                                                        // the Abstract
                                                        // component
                                                        System.out.println("EXTRACTION BEGINS "
                                                                        + nodenew11.getLocalName());
                                                        String componentCatalogQueryforActualInputsandOutputsforAbstractComponent = Queries
                                                                        .componentCatalogQueryforActualInputsandOutputsforAbstractComponent(
                                                                                        PREFIX_COMP_CATALOG,
                                                                                        nodenew11.getLocalName()
                                                                                                        + "Class");
                                                        rnew2 = null;
                                                        ...
                                                        while (rnew2.hasNext()) {
                                                                ...
                                                                if (nodenew.getLocalName()
                                                                                .equals(nodenew11.getLocalName())) {
                                                        ...
                                                        System.out.println(
                                                                        "EXTRACTED THE INPUTS AND OUTPUTS ABSTRACT COMPONENT BY HERE");

                                                        // EXPORTING THE USER WHO CREATED THE COMPONENT
                                                        this.dataProps(Constants.PROV_WAS_ATTRIBUTED_TO,
                                                                        nodenew11.getLocalName().toUpperCase() + "_V1",
                                                                        userName, XSDDatatype.XSDstring);

                                                        // EXPORTING THE FACT THAT CLASSNAME-CLASS IS A CLASSNAME
                                                        this.classIsaClass(
                                                                        nodenew11.getLocalName().toUpperCase()
                                                                                        + "_CLASSV1",
                                                                        nodenew11.getLocalName().toUpperCase() + "_V1");
                                                        classNames.put(nodenew11.getLocalName().toUpperCase(),
                                                                        nodenew11.getLocalName().toUpperCase()
                                                                                        + "_CLASSV1");

                                                        // IS CONCRETE EXPORTED
                                                        this.dataProps(Constants.COMPONENT_IS_CONCRETE,
                                                                        nodenew11.getLocalName().toUpperCase() + "_V1",
                                                                        compConcreteAbs + "",
                                                                        XSDDatatype.XSDboolean);

                                                        // RDFS LABEL EXPORTED for canonical instance
                                                        this.dataProps(Constants.RDFS_LABEL,
                                                                        nodenew11.getLocalName().toUpperCase() + "_V1",
                                                                        nodenew11.getLocalName(),
                                                                        XSDDatatype.XSDstring);

                                                        // RDFS LABEL EXPORTED for class
                                                        this.dataProps2(Constants.RDFS_LABEL,
                                                                        nodenew11.getLocalName().toUpperCase()
                                                                                        + "_CLASSV1",
                                                                        nodenew11.getLocalName(),
                                                                        XSDDatatype.XSDstring);
                                                        this.dataProps2(Constants.OWL_VERSION_INFO,
                                                                        nodenew11.getLocalName().toUpperCase()
                                                                                        + "_CLASSV1",
                                                                        "1", XSDDatatype.XSDstring);

                                                        // INPUTS-- EXPORTED
                                                        this.inputsOutputs(Constants.COMPONENT_HAS_INPUT, inputsAbsComp,
                                                                        nodenew11.getLocalName().toUpperCase() + "_V1");

                                                        // OUTPUTS-- EXPORTED
                                                        this.inputsOutputs(Constants.COMPONENT_HAS_OUTPUT,
                                                                        outputsAbsComp,
                                                                        nodenew11.getLocalName().toUpperCase() + "_V1");

                                                        if (specialFolder == true) {
                                                                // this is a bit dirty, should be another constant
                                                                this.dataProps("http://www.w3.org/2000/01/rdf-schema#subClassOf",
                                                                                nodenew11.getLocalName().toUpperCase()
                                                                                                + "_V1",
                                                                                AbstractSuperClass.getLocalName(),
                                                                                XSDDatatype.XSDstring);
                                                        ...
                                                        // STEP4: EXTRACT ONLY THE PARAMETERS IF THEY EXIST FOR ABSTRACT
                                                        // COMPONENTS
                                                        System.out.println(
                                                                        "INPUT PARAMETERS EXTRACTION PRINTING FOR ABSTARCT COMPONENTS");
                                                        ComponentVersion.Step4(inputsAbsComp, componentCatalog,
                                                                        taxonomyExport);

                                                        // STEP5: EXTRACT ONLY THE (INPUTS) DATA VARIABLES IF THEY EXIST
                                                        // FOR ABSTRACT
                                                        // COMPONENTS
                                                        System.out.println(
                                                                        "INPUT DATA EXTRACTION PRINTING FOR ABSTRACT COMPONENTS ONLY");
                                                        ComponentVersion.Step5(inputsAbsComp, componentCatalog,
                                                                        taxonomyExport);

                                                        // STEP6: EXTRACT ONLY THE (OUTPUTS) DATA VARIABLES IF THEY
                                                        // EXIST FOR ABSTRACT
                                                        // COMPONENTS
                                                        System.out.println(
                                                                        "OUTPUT DATA EXTRACTION PRINTING FOR ABSTRACT COMPONENTS");
                                                        ComponentVersion.Step6(outputsAbsComp, componentCatalog,
                                                                        taxonomyExport);

                                                }
                                                // Case2: Checking the components once for same Inputs and Outputs
                                                else if (!namesOfAbstractCompswithSameNames.isEmpty()) {
                                                        // this means we have found a list of similar abstract
                                                        // components in the
                                                        // Taxonomy Model
                                                        // now there are 2 points we have to consider here
                                                        // first we have to check the inputs and outputs of all the
                                                        // similar named
                                                        // abstract components in the Taxonomy Model
                                                        // if there is one with the same number of inputs and outputs
                                                        // then just break
                                                        // off
                                                        // if there is no one with the same inputs and outputs then we
                                                        // have to export a
                                                        // new one out

                                                        System.out.println(
                                                                        "NAMES FOR SIMILAR NAMED ABSTRACT COMPONENTS");
                                                        for (String x : namesOfAbstractCompswithSameNames)
                                                                System.out.println(x);

                                                        System.out.println(
                                                                        "THE INPUTS AND OUTPUTS IN THE CURRENT ABSTRACT COMPONENT ARE:");
                                                        ///
                                                        String componentCatalogQueryforActualInputsandOutputsforAbstractComponent = Queries
                                                                        .componentCatalogQueryforActualInputsandOutputsforAbstractComponent(
                                                                                        PREFIX_COMP_CATALOG,
                                                        ...
                                                        // NOW WE HAVE TO FIND OUT THE NUMBER OF INPUTS AND OUTPUTS IN
                                                        // EACH OF THE
                                                        // SIMILAR NAMED ABSTRACT COMPONENTS
                                                        boolean clarifyToExportAbstractComp = true;
                                                        for (String whatwehave : namesOfAbstractCompswithSameNames) {
                                                                String inputsandoutputsforsimilarnamedComps = Queries
                                                                                .TaxonomyExportQueryforSubclassCheckfinal();
                                                                rnew2 = null;
                                                                rnew2 = queryLocalTaxonomyModelRepository(
                                                                                inputsandoutputsforsimilarnamedComps);
                                                                ...
                                                                while (rnew2.hasNext()) {
                                                                        ...
                                                                        Resource input = qsnew.getResource("?i");
                                                                        Resource output = qsnew.getResource("?o");
                                                                        nodeInstance = qsnew.getResource("?n")
                                                                                        .getLocalName();
                                                                        nodeClass = qsnew.getResource("?x")
                                                                                        .getLocalName();

                                                                        System.out.println("Node inside case2: ?"
                                                                                        + nodenew.getLocalName());
                                                                        // System.out.println("x inside case2:
                                                                        // ?"+x.getLocalName());
                                                                ...
                                                                if (similarAbsInps.size() == inputsAbsComp.size()
                                                                                && similarAbsOuts
                                                                                                .size() == outputsAbsComp
                                                                                                                .size()) {

                                                                        // add to classNames the appropriate version of
                                                                        // the class that we have to point
                                                                        // to.
                                                                        classNames.put(nodeinstanceans.substring(0,
                                                                                        nodeinstanceans.lastIndexOf(
                                                                                                        "_")),
                                                                                        nodeclassans);
                                                                        ...
                                                                        System.out.println(
                                                                                        "There is a match and hence we don't have to export anything and BREAK NOW");
                                                                        clarifyToExportAbstractComp = false;
                                                                        break;
                                                                }
                                                        }

                                                        // Case3: Export since there are no components with same number
                                                        // of inputs and
                                                        // outputs
                                                        if (clarifyToExportAbstractComp == true) {
                                                                // BIG TASK TO DO HERE IS TO IDENTIFY THE LATEST VERSION
                                                                // OF THE COMPONENT OUT OF
                                                                // ALL OTHER COMPONENTS
                                                                // SO THAT WHEN WE ARE CREATING THE NEW ABSTRACT
                                                                // COMPONENT
                                                                // WE WANT TO LINK THE LAST AND CURRENT ABSTRACT
                                                                // COMPONENTS WITH THE CANONICAL
                                                                // INSTANCES
                                                                ...
                                                                for (String x : namesOfAbstractCompswithSameNames) {
                                                                        String temp = x.substring(x.lastIndexOf("_V"),
                                                                                        x.length());
                                                                ...
                                                                System.out
                                                                                .println("THE MOST LATEST VERSION IS : "
                                                                                                + finalversionforNewAbstractComponent);
                                                                // ABOVE TASK DONE BY HERE

                                                                // this means that the same Abstract Component is not
                                                                // present. So now we will
                                                                // export a fresh one.
                                                                ...
                                                                OntProperty propSelec255 = taxonomyExport
                                                                                .createOntProperty(
                                                                                                Constants.PROV_WAS_REVISION_OF);
                                                                Resource source255 = taxonomyExport
                                                                                .getResource(NEW_TAXONOMY_CLASS.replace(
                                                                                                "#", "/Component#")
                                                                                                + EncodingUtils.encode(
                                                                                                                nodenew11.getLocalName()
                                                                                                                                .toUpperCase()
                                                                                                                                + finalversionforNewAbstractComponent));
                                                                Individual instance255 = (Individual) source255
                                                                                .as(Individual.class);
                                                                if ((nodenew11.getLocalName().toUpperCase()
                                                                                + finalversionforLatestAbstractComponent)
                                                                                .contains("http://")) {// it is a URI
                                                                        instance255.addProperty(propSelec255,
                                                                                        NEW_TAXONOMY_CLASS.replace("#",
                                                                                                        "/Component#")
                                                                                                        + nodenew11.getLocalName()
                                                                                                                        .toUpperCase()
                                                                                                        + finalversionforLatestAbstractComponent);
                                                                ...
                                                                // start exporting the inputs and outputs for the
                                                                // abstract component and
                                                                // canonical instance

                                                                // extracting the actual inputs,outputs and other
                                                                // factors for the Abstract
                                                                // component
                                                                System.out.println("EXTRACTION BEGINS "
                                                                                + nodenew11.getLocalName());
                                                                String componentCatalogQueryforActualInputsandOutputsforAbstractComponent11 = Queries
                                                                                .componentCatalogQueryforActualInputsandOutputsforAbstractComponent(
                                                                                                PREFIX_COMP_CATALOG,
                                                                                                nodenew11.getLocalName()
                                                                                                                + "Class");
                                                                rnew2 = null;
                                                                rnew2 = queryComponentCatalog(
                                                                                componentCatalogQueryforActualInputsandOutputsforAbstractComponent11);
                                                                ...
                                                                while (rnew2.hasNext()) {
                                                                        ...
                                                                        if (nodenew.getLocalName().equals(
                                                                                        nodenew11.getLocalName())) {
                                                                                inputsAbsComp11.add(
                                                                                                input.getLocalName());
                                                                                outputsAbsComp11.add(
                                                                                                output.getLocalName());
                                                                                if (concr != null) {
                                                                                        compConcreteAbs11 = concr
                                                                                                        .getBoolean();
                                                                                }
                                                                        }
                                                                }
                                                                ...
                                                                for (String o : outputsAbsComp11)
                                                                        System.out.println("outputs are: " + o);
                                                                System.out.println(
                                                                                "isConcrete is: " + compConcreteAbs11);
                                                                System.out.println(
                                                                                "EXTRACTED THE INPUTS AND OUTPUTS ABSTRACT COMPONENT BY HERE");
                                                                ...
                                                                // EXPORTING THE FACT THAT CLASSNAME-CLASS IS A
                                                                // CLASSNAME
                                                                this.classIsaClass(
                                                                                nodenew11.getLocalName().toUpperCase()
                                                                                                + "_CLASS"
                                                                ...
                                                                // INPUTS-- EXPORTED
                                                                this.inputsOutputs(Constants.COMPONENT_HAS_INPUT,
                                                                                inputsAbsComp,
                                                                                nodenew11.getLocalName().toUpperCase()
                                                                                                + finalversionforNewAbstractComponent);
                                                                ...
                                                                if (specialFolder == true) {
                                                                        this.dataProps(Constants.COMPONENT_HAS_SUPERCLASS_FOLDER,
                                                                                        nodenew11.getLocalName()
                                                                                                        .toUpperCase()
                                                                                                        + finalversionforNewAbstractComponent,
                                                                                        AbstractSuperClass
                                                                ...
                                                                // STEP4: EXTRACT ONLY THE PARAMETERS IF THEY EXIST FOR
                                                                // ABSTRACT COMPONENTS
                                                                System.out.println(
                                                                                "INPUT PARAMETERS EXTRACTION PRINTING FOR ABSTRACT COMPONENTS");
                                                                ComponentVersion.Step4(inputsAbsComp, componentCatalog,
                                                                                taxonomyExport);

                                                                // STEP5: EXTRACT ONLY THE (INPUTS) DATA VARIABLES IF
                                                                // THEY EXIST FOR ABSTRACT
                                                                // COMPONENTS
                                                                System.out.println(
                                                                                "INPUT DATA EXTRACTION PRINTING FOR ABSTRACT COMPONENTS ONLY");
                                                                ComponentVersion.Step5(inputsAbsComp, componentCatalog,
                                                                ...
                                                                // STEP6: EXTRACT ONLY THE (OUTPUTS) DATA VARIABLES IF
                                                                // THEY EXIST FOR ABSTRACT
                                                                // COMPONENTS
                                                                System.out.println(
                                                                                "OUTPUT DATA EXTRACTION PRINTING FOR ABSTRACT COMPONENTS");
                                        ...
                                        ///////// ********STARTS***********////////////
                                        ///// **************THIS IS THE ENTIRE PART WHERE WE THE INCOMING COMPONENT IS A
                                        ///// CONCRETE COMPONENT**********////

                                        else if (hs1Concr.contains(className)
                                                        || hs1Concr.contains(className.substring(0,
                                                                        className.length() - 5))) {

                                                System.out.println("NOW UR INSIDE CONCRETE COMPONENT CASE");
                                                String taxonomyModelforConcreteComponent = Queries
                                                                .TaxonomyExportQueryforSubclassCheckfinalAgain();
                                                ResultSet r2new = null;
                                                r2new = queryLocalTaxonomyModelRepository(
                                                                taxonomyModelforConcreteComponent);
                                                ...
                                                while (r2new.hasNext()) {
                                                        ...
                                                        if (c.getBoolean()) {
                                                                if (nodenew.getLocalName().toLowerCase()
                                                                                .contains(concrComponent.getLocalName()
                                                                                                .substring(0, concrComponent
                                                                                                                .getLocalName()
                                                                                                                .length()
                                                                                                                - 5)
                                                                                                .toLowerCase())) {
                                                                        ...
                                                                        // based on that className
                                                                        // saving all such cases in a HashSet
                                                                        if (!namesOfConcreteCompswithSameNames.contains(
                                                                                        nodenew.getLocalName())
                                                                                        && !nodenew.getLocalName()
                                                                                                        .contains("CLASS")) {
                                                                                System.out.println("nodenew "
                                                                                                ...
                                                                                                + " c "
                                                                                                + c.getBoolean());
                                                                                namesOfConcreteCompswithSameNames.add(
                                                                                                nodenew.getLocalName());
                                                                        }
                                                ...
                                                System.out.println(
                                                                "THIS WILL IDENTIFY THE CONTENTS OF THE CONCRETE HASHSET");
                                                System.out.println();
                                                ...
                                                System.out.println(
                                                                "FIND THE NAMES OF SIMILAR ABSTRACT COMPONENTS IN THE TAXONOMY HIERARCHY MODEL");
                                                ...
                                                while (r2new.hasNext()) {
                                                        ...
                                                        if (!concrete.getBoolean()) {
                                                                if (nodenew.getLocalName().toLowerCase()
                                                                                .contains(AbstractSuperClass
                                                                                                .getLocalName()
                                                                                                .substring(0, AbstractSuperClass
                                                                                                                .getLocalName()
                                                                                                                .length()
                                                                                                                - 5)
                                                                                                .toLowerCase())) {
                                                                        ...
                                                                        // based on that className
                                                                        // saving all such cases in a HashSet
                                                                        if (!namesOfAbsCompswithSameNames.contains(
                                                                                        nodenew.getLocalName())) {
                                                                                namesOfAbsCompswithSameNames.add(
                                                                                                nodenew.getLocalName());
                                                                        }
                                                ...
                                                System.out.println(
                                                                "THIS WILL IDENTIFY THE CONTENTS OF THE ABSTRACT HASHSET");
                                                System.out.println();
                                                ...
                                                // IF THE SIZE OF THE HASHSET namesOfConcreteCompswithSameNames IS !=0
                                                // THEN ITS
                                                // DEALT WITH IN THE END
                                                // IF THE SIZE OF BOTH HASHSETS IS 0, THIS MEANS A FRESH NEW EXPORT _V1
                                                // IF THE SIZE OF namesOfConcreteCompswithSameNames==0 BUT
                                                // namesOfAbsCompswithSameNames!=0 THEN WE CHECK FURTHER
                                                // FOR INPS AND OUTPS
                                                if (namesOfAbsCompswithSameNames.size() != 0
                                                                && namesOfConcreteCompswithSameNames.size() == 0) {
                                                        System.out.println(
                                                                        "WE ARE HERE WHERE THE namesOfAbsCompswithSameNames.size()!=0 && namesOfConcreteCompswithSameNames.size()==0");
                                                        ...
                                                        for (String whatwehave : namesOfAbsCompswithSameNames) {
                                                                ...
                                                                while (rnew2.hasNext()) {
                                                                        ...
                                                                        if (nodenew.getLocalName().toUpperCase()
                                                                                        .equals(whatwehave)) {
                                                                                System.out.println(
                                                                                                "Node inside case2:? "
                                                                                                                + nodenew.getLocalName());
                                                                ...
                                                                System.out.println(
                                                                                "input size " + similarAbsInps.size());
                                                                System.out.println(
                                                                                "output size " + similarAbsOuts.size());

                                                                if (similarAbsInps.size() == hsi.size()
                                                                                && similarAbsOuts.size() == hso
                                                                                                .size()) {
                                                                        System.out.println(
                                                                                        "There is a match AND IT MEANS WE HAVE TO EXPORT THE CONCR COMP AS A SUBCLASS OF ABS COMP");
                                                        ...
                                                        if (clarifyToExportConcrComp == 1) {
                                                                ...
                                                                //
                                                                // //subclass relation
                                                                // this.addIndividualConcreteSubclass2(concrComponent.getLocalName().substring(0,concrComponent.getLocalName().length()-5).toUpperCase()+"_V1",
                                                                // Abs.getLocalName());
                                                                //
                                                                ModelUtils.addIndividualConcreteSubclass2(
                                                                                this.taxonomyExport,
                                                                                concrComponent.getLocalName()
                                                                                                .substring(0, concrComponent
                                                                                                                .getLocalName()
                                                                                                ...
                                                                                                .toUpperCase()
                                                                                                + "_CLASSV1",
                                                                                Abs.getLocalName().substring(0, Abs
                                                                                                .getLocalName()
                                                                                                .lastIndexOf("_"))
                                                                                                + "_CLASS"
                                                                                                + Abs.getLocalName()
                                                                                                                .substring(Abs.getLocalName()
                                                                                                                                .lastIndexOf("_")
                                                                                                                                + 1,
                                                                                                                                Abs.getLocalName()
                                                                                                                                                .length()));

                                                                // for the component blankNode extraction for HARDWARE
                                                                // and SOFTWARE
                                                                // dependencies:
                                                                String componentCatalogQuerydependencies = Queries
                                                                                .componentCatalogQuerydependencies(
                                                                                                PREFIX_COMP_CATALOG,
                                                                                                concrComponent.getLocalName()
                                                                                                                .substring(0,
                                                                                                                                concrComponent.getLocalName()
                                                                ...
                                                                if (rnew2.hasNext()) {
                                                                        ...
                                                                        try {
                                                                                Resource minversion = qsnew.getResource(
                                                                                                "?minversion");
                                                                                minversionval = minversion
                                                                                                .getLocalName();
                                                                                Literal needs = qsnew.getLiteral(
                                                                                                "?needs64bit");
                                                                                ...
                                                                                System.out.println(
                                                                                                "needs64bit " + needs);
                                                                                System.out.println("minversion is "
                                                                                                + minversionval);
                                                                ...
                                                                exportHardwareSoftwareDependencies(needsval, memval,
                                                                                stval, memval, concrComponent,
                                                                                versioncontrol);

                                                                // extracting the actual inputs,outputs and other
                                                                // factors for the component

                                                                String componentCatalogQueryforActualInputsandOutputsforComponent = Queries
                                                                                .componentCatalogQueryforActualInputsandOutputsforComponent(
                                                                                                PREFIX_COMP_CATALOG,
                                                                                                concrComponent.getLocalName());
                                                                rnew2 = null;
                                                                rnew2 = queryComponentCatalog(
                                                                ...
                                                                while (rnew2.hasNext()) {
                                                                        ...
                                                                        if (nodenew.getLocalName().equals(concrComponent
                                                                                        .getLocalName().substring(0,
                                                                                                        concrComponent.getLocalName()
                                                                                                                        .length()
                                                                                                                        - 5))) {
                                                                ...
                                                                try {
                                                                        this.dataProps(Constants.COMPONENT_HAS_MD5_CODE,
                                                                                        concrComponent.getLocalName()
                                                                                                        .substring(0, concrComponent
                                                                                                                        .getLocalName()
                                                                                                                        .length()
                                                                                                        ...
                                                                                                        .toUpperCase()
                                                                                                        + "_V1",
                                                                                        EncodingUtils.MD5ComponentCode(
                                                                                                        componentDirectory
                                                                                                                        + compLoc.substring(
                                                                                                                                        compLoc.lastIndexOf(
                                                                                                                                                        "\\")
                                                                ...
                                                                if (!doc11.equals("")) {
                                                                        this.dataProps(Constants.COMPONENT_HAS_DOCUMENTATION,
                                                                                        concrComponent.getLocalName()
                                                                                                        .substring(0, concrComponent
                                                                                                                        .getLocalName()
                                                                                                                        .length()
                                                                ...
                                                                this.dataProps2(Constants.RDFS_LABEL,
                                                                                concrComponent.getLocalName()
                                                                                                ...
                                                                                                .toUpperCase()
                                                                                                + "_V1",
                                                                                concrComponent.getLocalName().substring(
                                                                                                0,
                                                                                                concrComponent.getLocalName()
                                                                                                                .length()
                                                                                                                - 5),
                                                                ...
                                                                this.dataProps(Constants.COMPONENT_HAS_LOCATION,
                                                                                concrComponent.getLocalName()
                                                                                                .substring(0, concrComponent
                                                                                                                .getLocalName()
                                                                                                                .length()
                                                                ...
                                                                // STEP1: EXTRACT ONLY THE (INPUTS) PARAMETERS IF THEY
                                                                // EXIST FOR COMPONENTS
                                                                System.out.println(
                                                                                "INPUT PARAMETERS EXTRACTION PRINTING COMPONENTS");
                                                                ComponentVersion.Step1(hsi, componentCatalog,
                                                                                taxonomyExport);

                                                                // STEP2: EXTRACT ONLY THE (INPUTS) DATA VARIABLES IF
                                                                // THEY EXIST FOR COMPONENTS
                                                                System.out.println(
                                                                                "INPUT DATA EXTRACTION PRINTING COMPONENTS");
                                                                ComponentVersion.Step2(hsi, componentCatalog,
                                                                                taxonomyExport);

                                                                // STEP3: EXTRACT ONLY THE (OUTPUTS) DATA VARIABLES IF
                                                                // THEY EXIST FOR COMPONENTS
                                                                System.out.println(
                                                                                "OUTPUT DATA EXTRACTION PRINTING COMPONENTS");
                                                                ComponentVersion.Step3(hso, componentCatalog,
                                                                                taxonomyExport);

                                                        }
                                                        if (clarifyToExportConcrComp == 0) {
                                                                // this means we export both as a new version (LATEST
                                                                // VERSION) AND LINKING ALSO
                                                                // CANONICAL INSTANCES
                                                                ...
                                                                // AbstractSuperClass.getLocalName().substring(0,AbstractSuperClass.getLocalName().length()-5)+finalversionforNewAbstractComponent);
                                                                ...
                                                                // ModelUtils.addIndividualConcreteSubclass2(this.taxonomyExport,
                                                                // AbstractSuperClass.getLocalName().substring(0,AbstractSuperClass.getLocalName().length()-5)+"_CLASS"+finalversionforNewAbstractComponent.substring(1,finalversionforNewAbstractComponent.length()),"COMPONENT");

                                                                System.out.println(
                                                                                "EXPORTED THE BASIC CONCRETE COMPONENT BY HERE");
                                                                ...
                                                                Individual instance255 = (Individual) source255
                                                                                .as(Individual.class);
                                                                if ((AbstractSuperClass.getLocalName()
                                                                                .substring(0, AbstractSuperClass
                                                                                                .getLocalName().length()
                                                                                                - 5)
                                                                                .toUpperCase()
                                                                ...
                                                                while (rnew2.hasNext()) {
                                                                        ...
                                                                        Literal loc = qsnew.getLiteral("?loc");
                                                                        System.out.println(
                                                                                        "trying the nodenew comparison here: "
                                                                                                        + nodenew.getLocalName());
                                                                        if (nodenew.getLocalName().equals(concrComponent
                                                                                        .getLocalName().substring(0,
                                                                                                        concrComponent.getLocalName()
                                                                ...
                                                                System.out.println("isConcrete is: " + compConcrete);
                                                                // String
                                                                // compLoc2="/Users/Tirthmehta/Desktop/TestingDomain2/Component/";
                                                                ...
                                                                HashSet<String> outputsAbsComp = new HashSet<>();
                                                                boolean compConcreteAbs = false;
                                                                System.out.println("what we have " + AbstractSuperClass
                                                                                .getLocalName().substring(0,
                                                                                                AbstractSuperClass
                                                                                                                .getLocalName()
                                                                                                                .length()
                                                                ...
                                                                while (rnew2.hasNext()) {
                                                                        ...
                                                                        Literal concr = qsnew.getLiteral("?concr");
                                                                        Literal loc = qsnew.getLiteral("?loc");
                                                                        System.out.println("nodenew inside now: "
                                                                                        + nodenew.getLocalName());
                                                                        if (nodenew.getLocalName()
                                                                                        .equals(AbstractSuperClass
                                                                                                        .getLocalName()
                                                                                                        .substring(0,
                                                                                                                        AbstractSuperClass
                                                                                                                                        .getLocalName()
                                                                ...
                                                                // EXPORTING THE FACT THAT CLASSNAME-CLASS IS A
                                                                // CLASSNAME
                                                                this.classIsaClass(AbstractSuperClass.getLocalName()
                                                                                .substring(0, AbstractSuperClass
                                                                                                .getLocalName().length()
                                                                                                - 5)
                                                                                .toUpperCase()
                                                                                ...
                                                                                AbstractSuperClass.getLocalName()
                                                                                                ...
                                                                                                .toUpperCase()
                                                                                                + finalversionforNewAbstractComponent);
                                                                classNames.put(AbstractSuperClass.getLocalName()
                                                                                .substring(0, AbstractSuperClass
                                                                                                .getLocalName().length()
                                                                                                - 5)
                                                                                .toUpperCase(),
                                                                ...
                                                                // RDFS LABEL EXPORTED for class
                                                                this.dataProps2(Constants.RDFS_LABEL, AbstractSuperClass
                                                                                .getLocalName()
                                                                                .substring(0, AbstractSuperClass
                                                                                                .getLocalName().length()
                                                                                                - 5)
                                                                ...
                                                                System.out.println(
                                                                                "INPUT PARAMETERS EXTRACTION PRINTING FOR ABSTARCT COMPONENTS");
                                                                ComponentVersion.Step4(inputsComp, componentCatalog,
                                                                                taxonomyExport);
                                                                ...
                                                                System.out.println(
                                                                                "INPUT DATA EXTRACTION PRINTING FOR ABSTRACT COMPONENTS ONLY");
                                                                ComponentVersion.Step5(inputsComp, componentCatalog,
                                                                                taxonomyExport);
                                                                ...
                                                                System.out.println(
                                                                                "OUTPUT DATA EXTRACTION PRINTING FOR ABSTRACT COMPONENTS");
                                                                ComponentVersion.Step6(outputsComp, componentCatalog,
                                                                                taxonomyExport);
                                                ...
                                                // Case2: Direct Export since the size of the found concr comps with
                                                // similar
                                                // names is 0
                                                if (namesOfConcreteCompswithSameNames.isEmpty()
                                                                && namesOfAbsCompswithSameNames.isEmpty()) {
                                                        ...
                                                        // this.addIndividualConcreteSubclass2(concrComponent.getLocalName().substring(0,concrComponent.getLocalName().length()-5).toUpperCase()+"_V1"
                                                        // ,
                                                        // AbstractSuperClass.getLocalName().substring(0,AbstractSuperClass.getLocalName().length()-5)+"_V1");
                                                        ...
                                                        // ModelUtils.addIndividualConcreteSubclass2(this.taxonomyExport,
                                                        // AbstractSuperClass.getLocalName().substring(0,AbstractSuperClass.getLocalName().length()-5)+"_CLASSV1","COMPONENT");

                                                        System.out.println(
                                                                        "EXPORTED THE BASIC CONCRETE COMPONENT BY HERE");
                                                        // export of ABSTRACT component ends

                                                        // for the component blankNode extraction for HARDWARE and
                                                        // SOFTWARE
                                                        // dependencies:
                                                        String componentCatalogQuerydependencies = Queries
                                                                        .componentCatalogQuerydependencies(
                                                        ...
                                                        if (rnew2.hasNext()) {
                                                                ...
                                                                try {
                                                                        Resource minversion = qsnew
                                                                                        .getResource("?minversion");
                                                        ...
                                                        // EXPORTING THE MD5 FOR THE COMPONENT CODE
                                                        try {
                                                                System.out.println(componentDirectory
                                                                                + compLoc.substring(
                                                                                                compLoc.lastIndexOf("/")
                                                                                                                + 1,
                                                                                                compLoc.length())
                                                        ...
                                                        this.dataProps(Constants.COMPONENT_IS_CONCRETE,
                                                                        concrComponent.getLocalName()
                                                                                        ...
                                                                                        .toUpperCase()
                                                                                        + "_V1",
                                                                        compConcrete + "", XSDDatatype.XSDboolean);
                                                        ...
                                                        // INPUTS-- EXPORTED
                                                        this.inputsOutputs(Constants.COMPONENT_HAS_INPUT, inputsComp,
                                                                        concrComponent.getLocalName()
                                                                                        .substring(0, concrComponent
                                                                                                        .getLocalName()
                                                                                                        .length() - 5)
                                                        ...
                                                        // OUTPUTS-- EXPORTED
                                                        this.inputsOutputs(Constants.COMPONENT_HAS_OUTPUT, outputsComp,
                                                                        concrComponent.getLocalName()
                                                                                        .substring(0, concrComponent
                                                                                                        .getLocalName()
                                                                                                        .length() - 5)
                                                        ...
                                                        // EXPORTING THE HARDWARE AND SOFTWARE DEPENDENCY
                                                        int versioncontrol = 1;
                                                        exportHardwareSoftwareDependencies(needsval, memval, stval,
                                                                        memval, concrComponent,
                                                                        versioncontrol);
                                                        ...
                                                        while (rnew2.hasNext()) {
                                                                ...
                                                                System.out.println("nodenew inside now: "
                                                                                + nodenew.getLocalName());
                                                                if (nodenew.getLocalName().equals(AbstractSuperClass
                                                                                .getLocalName().substring(0,
                                                                                                AbstractSuperClass
                                                                                                                .getLocalName()
                                                                                                                .length()
                                                        ...
                                                        // EXPORTING THE FACT THAT CLASSNAME-CLASS IS A CLASSNAME
                                                        this.classIsaClass(
                                                                        AbstractSuperClass.getLocalName().toUpperCase()
                                                                                        .substring(0,
                                                                                                        AbstractSuperClass
                                                                                                                        .getLocalName()
                                                                                                                        .length()
                                                        ...
                                                        this.dataProps(Constants.COMPONENT_IS_CONCRETE,
                                                                        AbstractSuperClass.getLocalName()
                                                                                        ...
                                                                                        .toUpperCase()
                                                                                        + "_V1",
                                                                        compConcreteAbs + "", XSDDatatype.XSDboolean);
                                                ...
                                                // Case2: Checking the components once for same Inputs and Outputs
                                                else if (namesOfConcreteCompswithSameNames.size() != 0
                                                                && namesOfAbsCompswithSameNames.size() != 0) {

                                                        System.out.println(
                                                                        "WE ARE IN THIS CASE NOW WHERE THE CONCRETE COMP'S INTERNAL FACTORS HAVE CHANGED-----");
                                                        // this means we have found a list of similar abstract
                                                        // components in the
                                                        // Taxonomy Model
                                                        // now there are 2 points we have to consider here
                                                        ...
                                                        System.out.println(
                                                                        "NAMES FOR SIMILAR NAMED CONCRETE COMPONENTS");
                                                        for (String x : namesOfConcreteCompswithSameNames)
                                                                System.out.println(x);

                                                        System.out.println(
                                                                        "THE INPUTS AND OUTPUTS IN THE CURRENT CONCRETE COMPONENT ARE:");
                                                        ///
                                                        String componentCatalogQueryforActualInputsandOutputsforConcreteComponent = Queries
                                                                        .componentCatalogQueryforActualInputsandOutputsforComponent(
                                                                                        PREFIX_COMP_CATALOG,
                                                                                        nodenew11.getLocalName()
                                                                                                        + "Class");
                                                        rnew2 = null;
                                                        rnew2 = queryComponentCatalog(
                                                                        componentCatalogQueryforActualInputsandOutputsforConcreteComponent);
                                                        ...
                                                        while (rnew2.hasNext()) {
                                                                ...
                                                                Resource output = qsnew.getResource("?o");
                                                                Literal loc = qsnew.getLiteral("?loc");
                                                                System.out.println("nodenew now case2: "
                                                                                + nodenew.getLocalName());
                                                                System.out.println("concrComp now case2: "
                                                                                + concrComponent.getLocalName());
                                                                if (nodenew.getLocalName().equals(concrComponent
                                                                                .getLocalName().substring(0,
                                                                                                concrComponent.getLocalName()
                                                                                                                .length()
                                                                                                                - 5))) {
                                                                        inputsCurrentConcrComp
                                                                                        .add(input.getLocalName());
                                                                        outputsCurrentConcrComp
                                                                                        .add(output.getLocalName());
                                                                }
                                                                if (loc != null)
                                                                        compLoc = loc.getString();
                                                        }
                                                        ...
                                                        // NOW WE HAVE TO FIND OUT THE NUMBER OF INPUTS AND OUTPUTS IN
                                                        // EACH OF THE
                                                        // SIMILAR NAMED CONCRETE COMPONENTS
                                                        int clarifyToExportConcrComp = 0,
                                                                        codeisdifferentbutinputsandoutputsaresame = 0;
                                                        for (String whatwehave : namesOfConcreteCompswithSameNames) {
                                                                ...
                                                                while (rnew2.hasNext()) {
                                                                        ...
                                                                        if (nodenew.getLocalName().toUpperCase()
                                                                                ...
                                                                                System.out.println("x inside case2: ? "
                                                                                                + x.getLocalName());
                                                                                // System.out.println("y inside case2: ?
                                                                                // "+y.getLocalName());
                                                                                System.out.println("input is :"
                                                                                                + input.getLocalName());
                                                                                System.out.println("output is :"
                                                                ...
                                                                try {
                                                                        System.out.println(componentDirectory
                                                                                        + compLoc.substring(compLoc
                                                                                                        ...
                                                                                                        compLoc.length())
                                                                                        + ".zip");
                                                                        one = EncodingUtils.MD5ComponentCode(
                                                                                        componentDirectory
                                                                                                        + compLoc.substring(
                                                                                                                        compLoc.lastIndexOf(
                                                                                                                                        "/")
                                                                ...
                                                                if (similarAbsInps.size() == inputsCurrentConcrComp
                                                                                .size()
                                                                                && similarAbsOuts
                                                                                                .size() == outputsCurrentConcrComp
                                                                                                                .size()) {
                                                                        if (one != null && two != null
                                                                                        && one.equals(two)) {
                                                                                classNames.put(nodeInstanceans
                                                                                                .substring(0, nodeInstanceans
                                                                                                                .lastIndexOf("_")),
                                                                                                nodeClassans);
                                                                                for (String x : classNames.keySet())
                                                                                        System.out.println(x + " "
                                                                        ...
                                                                        // the else here means that the inputs and
                                                                        // outputs are the same but the code is
                                                                        // different
                                                        ...
                                                        if (clarifyToExportConcrComp == 1)
                                                                System.out.println(
                                                                                "There is a match CONCRETE COMPONENT CASE and BREAKKKKKKKKKKK!!!!!!!!!!!!!!");
                                                        else if (clarifyToExportConcrComp == 0
                                                                        && codeisdifferentbutinputsandoutputsaresame == 1) {
                                                                System.out.println(
                                                                                "we are in the case where the inputs and outputs are the same but the code is different so we export as a subclass with latest version");
                                                                String finalversionforNewAbstractComponent = "";
                                                                String finalversionforLatestAbstractComponent = "";
                                                                int max = Integer.MIN_VALUE;
                                                                for (String x : namesOfConcreteCompswithSameNames) {
                                                                ...
                                                                // EXPORTING THE PROV_WAS_REVISION_OF
                                                                OntProperty propSelec256 = taxonomyExport
                                                                                .createOntProperty(
                                                                                                Constants.PROV_WAS_REVISION_OF);
                                                                Resource source256 = taxonomyExport
                                                                                .getResource(NEW_TAXONOMY_CLASS.replace(
                                                                                                "#", "/Component#")
                                                                                                + EncodingUtils.encode(
                                                                                                                concrComponent.getLocalName()
                                                                                                                                .substring(0,
                                                                                                                                                concrComponent.getLocalName()
                                                                                                                                                                ...
                                                                                                                                                                - 5)
                                                                                                                                + finalversionforNewAbstractComponent));
                                                                Individual instance256 = (Individual) source256
                                                                                .as(Individual.class);
                                                                if ((concrComponent.getLocalName().substring(0,
                                                                                concrComponent.getLocalName().length()
                                                                                                - 5)
                                                                                + finalversionforLatestAbstractComponent)
                                                                                .contains("http://")) {// it is a URI
                                                                        instance256.addProperty(propSelec256,
                                                                                        NEW_TAXONOMY_CLASS.replace("#",
                                                                                                        "/Component#")
                                                                                                        + concrComponent.getLocalName()
                                                                                                                        .substring(0,
                                                                ...
                                                                ComponentVersion.Step3(hso, componentCatalog,
                                                                        ...
                                                                        && codeisdifferentbutinputsandoutputsaresame == 0) {
                                                                System.out.println(
                                                                                "NOW CHECK IF THE INPUTS AND OUTPUTS WERE THE SAME AND ONLY THE CODE HAD CHANGED...IF YES THEN WE EXPORT ONLY A SUBCLASS FACTOR");

                                                                System.out.println(
                                                                                "There is NO MATCH AND HENCE WE HAVE TO CREATE A VERSIONING HERE");
                                                                System.out.println("THIS STARTS THE FIRST CASE PART-2");
                                                                                ...
                                                                                + AbstractSuperClass.getLocalName()
                                                                                                .substring(0,
                                                                                                                AbstractSuperClass
                                                                                                                                ...
                                                                                                                                .length()
                                                                                                                                - 5));
                                                                String componentCatalogQueryforActualInputsandOutputsforAbstractComponent112 = Queries
                                                                                .componentCatalogQueryforActualInputsandOutputsforAbstractComponent(
                                                                                                PREFIX_COMP_CATALOG,
                                                                                                AbstractSuperClass
                                                                                                                .getLocalName()
                                                                ...
                                                                rnew2 = null;
                                                                rnew2 = queryComponentCatalog(
                                                                                componentCatalogQueryforActualInputsandOutputsforAbstractComponent112);
                                                                                                ...
                                                                                                "classisaclass "
                                                                                                                ...
                                                                                                                + finalversionforNewAbstractComponent
                                                                                                                                .substring(1,
                                                                                                                                                finalversionforNewAbstractComponent
                                                                                                                                                                .length()));
                                                                System.out.println("secondpara " + AbstractSuperClass
                                                                                .getLocalName()
                                                                                .substring(0, AbstractSuperClass
                                                                                                .getLocalName().length()
                                                                                                - 5)
                        ...
                        if (typeComp.isURIResource()) { // only adds the type if the type is a uRI (not a blank node)
                                ...
                                String newchangeforthetype = typeComp.getURI().substring(
                                                typeComp.getURI().lastIndexOf("/"),
                                                typeComp.getURI().length());
                                String newchangeforthetype2 = newchangeforthetype.substring(
                                                newchangeforthetype.lastIndexOf("#"),
                                                newchangeforthetype.length());
                                String tempURI = EncodingUtils.encode(
                                                Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + newTemplateName_
                                                                + res.getLocalName());
                                ...
                                OntClass cAux = OPMWModel
                                                .createClass(NEW_TAXONOMY_CLASS_2 + "Component" + newchangeforthetype2);// repeated
                                                                                                                        // tuples
                                                                                                                        // will
                                                                                                                        // not
                                                                                                                        // be
                        ...
                        if (rule != null) {
                                ...
                                ModelUtils.addDataProperty(OPMWModel,
                                                Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + newTemplateName_
                                                                + res.getLocalName(),
                                                rule.getString(),
                                                Constants.WINGS_DATA_PROP_HAS_RULE);

                                // ModelUtils.addDataProperty(OPMWModel,
                                // Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS+"/"+newTemplateName_+res.getLocalName(),rule.getString(),
                                // Constants.OPMW_COMPONENT_HAS_RULES);
                        }
                        if (isConcrete != null) {
                                System.out.println("is component: " + comp.getLocalName() + " concrete: "
                                                + isConcrete.getBoolean());
                                ModelUtils.addDataProperty(OPMWModel,
                                                Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + newTemplateName_
                                                                + res.getLocalName(),
                                                isConcrete.getBoolean() + "", Constants.OPMW_DATA_PROP_IS_CONCRETE,
                                                XSDDatatype.XSDboolean);
                        }
                        ModelUtils.addProperty(OPMWModel,
                                        Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + newTemplateName_
                ...
                while (r.hasNext()) {
                        ...
                        ModelUtils.addIndividual(OPMWModel, newTemplateName_ + variable.getLocalName(),
                                        Constants.OPMW_DATA_VARIABLE, "Data variable " + variable.getLocalName());
                        // p-plan interop
                        OntClass cVar = OPMWModel.createClass(Constants.P_PLAN_Variable);
                        cVar.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_DATA_VARIABLE + "/"
                                        + EncodingUtils.encode(newTemplateName_ + variable.getLocalName()));

                        // we add the individual as a workflowTemplateArtifact as well
                        String aux = EncodingUtils
                                        .encode(Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_
                                                        + variable.getLocalName());
                        OntClass cAux = OPMWModel.createClass(Constants.OPMW_WORKFLOW_TEMPLATE_ARTIFACT);// repeated
                                                                                                         // tuples will
                                                                                                         // not
                                                                                                         // be
                                                                                                         // duplicated
                        ...
                        if (dim != null) {// sometimes is null, but it shouldn't
                                ModelUtils.addDataProperty(OPMWModel,
                                                Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_
                                                                + variable.getLocalName(),
                                                "" + dim.getInt(), Constants.OPMW_DATA_PROP_HAS_DIMENSIONALITY,
                                                XSDDatatype.XSDint);
                                ...
                                ModelUtils.addDataProperty(OPMWModel,
                                                Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_
                                                                + variable.getLocalName(),
                                                "" + isCollection, Constants.OPMW_DATA_PROP_IS_COLLECTION,
                                                XSDDatatype.XSDboolean);
                        }
                        ...
                        if (type != null) {
                                ...
                                if (type.isURIResource()) {
                                        ...
                                        String newchangeforthetype = type.getURI().substring(
                                                        type.getURI().lastIndexOf("/"),
                                                        type.getURI().length());
                                        String newchangeforthetype2 = newchangeforthetype.substring(
                                                        newchangeforthetype.lastIndexOf("#"),
                                                        newchangeforthetype.length());
                                        String nameEncoded = EncodingUtils
                                                        .encode(Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_
                                                                        + variable.getLocalName());
                                        OntClass c = OPMWModel.createClass(
                                                        NEW_TAXONOMY_CLASS_2 + "Data" + newchangeforthetype2);// repeated
                                                                                                              // tuples
                                                                                                              // will
                                                                                                              // not
                                                                                                              // be
                ...
                while (r.hasNext()) {
                        ...
                        ModelUtils.addIndividual(OPMWModel, newTemplateName_ + res.getLocalName(),
                                        Constants.OPMW_PARAMETER_VARIABLE, "Parameter variable " + res.getLocalName());
                        // p-plan interop
                        OntClass cVar = OPMWModel.createClass(Constants.P_PLAN_Variable);
                        cVar.createIndividual(
                                        Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_PARAMETER_VARIABLE + "/"
                                                        + EncodingUtils.encode(newTemplateName_ + res.getLocalName()));

                        // add the parameter value as an artifact too
                        String aux = EncodingUtils
                                        .encode(Constants.CONCEPT_PARAMETER_VARIABLE + "/" + newTemplateName_
                                                        + res.getLocalName());
                        OntClass cAux = OPMWModel.createClass(Constants.OPMW_WORKFLOW_TEMPLATE_ARTIFACT);// repeated
                                                                                                         // tuples will
                                                                                                         // not
                        ...
                        ModelUtils.addProperty(OPMWModel,
                                        Constants.CONCEPT_PARAMETER_VARIABLE + "/" + newTemplateName_
                                                        + res.getLocalName(),
                                        Constants.CONCEPT_WORKFLOW_TEMPLATE + "/" + newTemplateName,
                ...
                while (r.hasNext()) {
                        ...
                        if (role != null) {
                                System.out.println(
                                                "Node " + resNode.getLocalName() + " Uses " + resVar.getLocalName()
                                                                + " Role: " + role);
                                ...
                                ModelUtils.createSubProperty(OPMWModel, Constants.OPMW_PROP_USES,
                                                Constants.PREFIX_EXTENSION + "usesAs_" + role);
                                // description of the new property
                                OntProperty propUsed = OPMWModel
                                                .getOntProperty(Constants.PREFIX_EXTENSION + "usesAs_" + role);
                                propUsed.addLabel("Property that indicates that a resource has been used as a " + role,
                                                "EN");
                        }
                }
                ...
                while (r.hasNext()) {
                        ...
                        if (role != null) {
                                ...
                                propUsed.addLabel("Property that indicates that a resource has been used as a " + role,
                                                "EN");
                                // System.out.println(resVar.getLocalName() +" type "+
                                // qs.getResource("?t").getURI());
                        }
                }
                ...
                while (r.hasNext()) {
                        ...
                        if (role != null) {
                                System.out.println("Artifact " + resVar.getLocalName() + " Is generated by node "
                                                + resNode.getLocalName() + " Role " + role);
                                // add the roles as subproperty of used. This triple should be on the ontology.
                                ModelUtils.addProperty(OPMWModel,
                                                Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_
                                                                + resVar.getLocalName(),
                                ...
                                ModelUtils.createSubProperty(OPMWModel, Constants.OPMW_PROP_IGB,
                                                Constants.PREFIX_EXTENSION + "isGeneratedByAs_" + role);
                                OntProperty propGenerated = OPMWModel
                                                .getOntProperty(Constants.PREFIX_EXTENSION + "isGeneratedByAs_" + role);
                                propGenerated.addLabel(
                                                "Property that indicates that a resource has been generated as a "
                                                                + role,
                                                "EN");
                        }
                }
                ...
                while (r.hasNext()) {
                        ...
                        if (roleOrig != null && roleDest != null) {
                                System.out
                                                .println("Artifact " + resVar.getLocalName() + " is generated by node "
                                                                + resNode.getLocalName()
                                                                + " with role " + roleOrig + " and uses node "
                                                                + resNodeD.getLocalName()
                                                                + " with role " + roleDest);
                        }
                        ...
                        if (roleOrig != null) {
                                ...
                                ModelUtils.createSubProperty(OPMWModel, Constants.OPMW_PROP_IGB,
                                                Constants.PREFIX_EXTENSION + "isGeneratedByAs_" + roleOrig);
                                OntProperty propGenerated = OPMWModel
                                                .getOntProperty(Constants.PREFIX_EXTENSION + "isGeneratedByAs_"
                                                                + roleOrig);
                                propGenerated.addLabel(
                                                "Property that indicates that a resource has been generated as a "
                                                                + roleOrig,
                        ...
                        if (roleDest != null) {
                                ...
                                // Constants.PREFIX_ONTOLOGY_PROFILE+"used_"+roleDest.getLocalName());
                                ModelUtils.addProperty(OPMWModel,
                                                Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + newTemplateName_
                                                                + resNodeD.getLocalName(),
                                                Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_
                                ...
                                ModelUtils.createSubProperty(OPMWModel, Constants.OPMW_PROP_USES,
                                                Constants.PREFIX_EXTENSION + "usesAs_" + roleDest);
                                OntProperty propUsed = OPMWModel
                                                .getOntProperty(Constants.PREFIX_EXTENSION + "usesAs_" + roleDest);
                                propUsed.addLabel("Property that indicates that a resource has been used as a "
                                                + roleDest, "EN");
                        }
                }
         ...
         * Note that this method will load the Workflow Instance and Workflow Expanded
         * Template
         * from the data in the execution file. It is assumed that these URLs are
         * accessible.
         *
         * @param resultFile           the Wings Execution File for this execution.
         * @param libraryFile          the library file containing all the execution
         *                             metadata.
         ...
         * @param suffix               id to be added to identify uniquely certain
         *                             resources
         * @param data_catalog         Path to the data catalog export file.
         * @param exportMode           mode in which we want to export the produced
         *                             files (TURTLE, RDF/XML, etc.)
         * @param dataCatalogDirectory path to the WINGS data directory where all data
         *                             files are present
         * @param domainName           name of the domain of the workflow
         * @param componentCatalogPath component catalog path to export the right
         *                             expanded templates.
         * @param componentDirectory
         * @return The URI for the template. An empty string will be returned if an
         *         error occurred (see console for details)
         * @throws IOException
         */
        // public String transformWINGSResultsToOPMW(String resultFile, String
        // libraryFile, String modeFile, String outFilenameOPMW, String
        // outFilenamePROV){
        public String transformWINGSResultsToOPMW(String resultFile, String libraryFile, String modeFile,
                        ...
                        String exportMode,
                        String dataCatalogDirectory, String domainName, String componentCatalogPath,
                        String componentDirectory) throws IOException {
                ...
                // now, extract the expanded template and the workflow instance. Load them as
                // well
                ...
                String expanded = "";// Auxiliary string so we don't have to continually ask which template we are
                                     // linking to
                ...
                // this.addIndividual(OPMWModel,"Account"+date,
                // Constants.OPMW_WORKFLOW_EXECUTION_ACCOUNT,"Execution account created on
                // "+date);
                ModelUtils.addIndividual(OPMWModel, "Account-" + suffix, Constants.OPMW_WORKFLOW_EXECUTION_ACCOUNT,
                                "Execution account created on " + date);
                // we also assert that it is an account
                // String accname =
                // EncodingUtils.encode(Constants.CONCEPT_WORKFLOW_EXECUTION_ACCOUNT+"/"+"Account"+date);
                String accname = EncodingUtils
                                .encode(Constants.CONCEPT_WORKFLOW_EXECUTION_ACCOUNT + "/" + "Account-" + suffix);
                OntClass cAux = OPMWModel.createClass(Constants.OPM_ACCOUNT);
                cAux.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + accname);

                // assessing whether to generate an expanded template (needs the template model
                // to be loaded)
                ...
                // this.loadExpandedTemplateFileToLocalRepository(expandedTemplateURI,
                // modeFile);
                hashedTemplateName = HashCreator.getAbstractTemplateHash(templateName, this.templateModel);
                if (generateExpandedTemplate) {
                        hashedExpandedTemplateName = createExpandedTemplate(accname, expandedTemplateName,
                                        expandedTemplateURI,
                                        hashedTemplateName, domainName, componentCatalogPath, componentDirectory);
                        templateToLink = hashedExpandedTemplateName;
                        expanded = "Expanded_";
                        // all the links have to be done to the expanded template instead of the
                        // template
                } else {
                ...
                if (user != null) {
                        ModelUtils.addIndividual(OPMWModel, user, Constants.OPM_AGENT, "Agent " + user);// user HAS to
                                                                                                        // have a URI
                ...
                ModelUtils.addDataProperty(OPMWModel, accname,
                                executionFile, Constants.OPMW_DATA_PROP_HAS_ORIGINAL_EXECUTION_FILE,
                                XSDDatatype.XSDanyURI);
                ...
                // status
                ModelUtils.addDataProperty(OPMWModel, accname,
                                status, Constants.OPMW_DATA_PROP_STATUS);
                // startTime
                ModelUtils.addDataProperty(OPMWModel, accname,
                ...
                // String
                // newexpandedtemplatename=newExpandedTemplateName.substring(0,newExpandedTemplateName.indexOf('-'));
                ...
                while (r.hasNext()) {
                        ...
                        System.out.println(
                                        "after derived from " + stepName + "\n\t " + sStartT + "\n\t " + sEndT + "\n\t "
                                                        + sStatus + "\n\t " + sCode);
                        // add each step with its metadata to the model Start and end time are reused
                        // from prov.
                        ModelUtils.addIndividual(OPMWModel, stepName + date, Constants.OPMW_WORKFLOW_EXECUTION_PROCESS,
                                        "Execution process " + stepName);
                        // add type opmv:Process as well
                        String auxP = EncodingUtils
                                        .encode(Constants.CONCEPT_WORKFLOW_EXECUTION_PROCESS + "/" + stepName + date);
                        ...
                        ModelUtils.addDataProperty(OPMWModel,
                                        Constants.CONCEPT_WORKFLOW_EXECUTION_PROCESS + "/" + stepName + date,
                                        sStatus,
                                        Constants.OPMW_DATA_PROP_STATUS);
                        ...
                        Resource blankNode = OPMWModel.createResource(
                                        Constants.OPMW_PROP_EXECUTABLE_COMPONENT + "/" + stepName
                                                        + executableComponentName);
                        blankNode.addProperty(OPMWModel.createOntProperty(Constants.OPMW_DATA_PROP_HAS_LOCATION),
                                        sCode).addProperty(OPMWModel.createOntProperty(Constants.RDFS_LABEL),
                                                        "Executable Component associated to " + stepName);
                        String procURI = Constants.PREFIX_EXPORT_RESOURCE + EncodingUtils
                                        .encode(Constants.CONCEPT_WORKFLOW_EXECUTION_PROCESS + "/" + stepName
                                                        + executableComponentName);
                        OPMWModel.getResource(procURI).addProperty(
                                        OPMWModel.createOntProperty(Constants.OPMW_PROP_HAS_EXECUTABLE_COMPONENT),
                                        blankNode);

                        // link node to the process templates
                        ModelUtils.addProperty(OPMWModel,
                                        Constants.CONCEPT_WORKFLOW_EXECUTION_PROCESS + "/" + stepName + date,
                                        Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + expanded + templateToLink
                                                        + "_" + stepName,
                ...
                // creating a hashmap to store the input vs the new name for it to make it
                ...
                while (r.hasNext()) {
                        ...
                        System.out.println("Step: " + step2 + " used input " + input2 + " with data binding: "
                                        + inputBinding2);
                        // need to open the file here:

                        System.out.println("hashing the input file here ");
                        File f = new File(dataCatalogDirectory
                                        + inputBinding2.substring(inputBinding2.lastIndexOf("/") + 1,
                                                        inputBinding2.length()));
                        System.out.println("opening file" + dataCatalogDirectory
                                        + inputBinding2.substring(inputBinding2.lastIndexOf("/") + 1,
                                                        inputBinding2.length()));
                        ArrayList<String> arr = new ArrayList<>();
                        StringBuilder sb = new StringBuilder();
                        String currentMD5 = null;
                        // IF THE FILE Does not exist, it might be an intermediate file, of whose
                        // versioning we do not take care of
                        if (!f.exists()) {
                        ...
                        try {
                                ...
                                try {
                                        ...
                                        System.out.println(
                                                        "Hashing is not possible for file size and hence we will not version it");
                                        continue;
                                }
                        } catch (Exception e) {
                        }
                        ...
                        while (r222.hasNext()) {
                                ...
                                // metadata
                                if (prop.contains("rdf-schema#type")
                                                || prop.contains("http://www.w3.org/2000/01/rdf-schema#comment")
                                                || prop.contains("rdf-syntax-ns#type")) {
                                        // the objects are resources in this case
                                        // String auxP =
                                        // EncodingUtils.encode(Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT+"/"+var+date);
                                        ...
                                        if (var.equals(input2)) {
                                                ...
                                                hsprops.put(temp.substring(temp.lastIndexOf("#") + 1, temp.length()),
                                                                obj);
                                                propertiesfromExecutionFile.add(prop
                                                                .substring(prop.lastIndexOf("/") + 1, prop.length()));
                                        }
                                }
                        ...
                        while (rnew123.hasNext()) {
                                ...
                                if (res.getLocalName().substring(0, res.getLocalName().indexOf("_"))
                                                .equals(inputBinding2
                                                                .substring(inputBinding2.lastIndexOf("/") + 1,
                                                                                inputBinding2.length())
                                                                .toUpperCase())) {
                                ...
                                try {
                                        ...
                                        if (prop112 != null && obj112 != null) {
                                                System.out.println("prop112 " + prop112.getLocalName() + " obj112 "
                                                                + obj112.getString());
                                                if (!prop112.getLocalName().equals("type")
                                                                && !prop112.getLocalName().equals("wasRevisionOf")) {
                                        ...
                                        if (prop112 != null && obj112 != null) {
                                                ...
                                                if (!prop112.getLocalName().equals("type")
                                                                && !prop112.getLocalName().equals("wasRevisionOf")) {
                                                        if (hmapnewone.containsKey(res.getLocalName())) {
                                                                ArrayList<String> temp = hmapnewone
                                                                                .get(res.getLocalName());
                        ...
                        if (similarNamesofInputFiles.isEmpty()) {
                                ...
                                hmapforInputstoVersions.put(input2,
                                                inputBinding2.substring(inputBinding2.lastIndexOf("/") + 1,
                                                                inputBinding2.length())
                                                                .toUpperCase() + "_" + currentMD5 + "_V1");

                                System.out.println("no match and create a new version v1");

                                String nameOfIndividualEnc = EncodingUtils
                                                .encode(inputBinding2
                                                                .substring(inputBinding2.lastIndexOf("/") + 1,
                                                                                inputBinding2.length())
                                                                .toUpperCase() + "_" + currentMD5 + "_V1");
                                OntClass c = dataCatalog
                                                .createClass(Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT);
                                c.createIndividual(Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT + "/"
                                                + nameOfIndividualEnc.toUpperCase());

                                OntProperty propSelec22;
                                propSelec22 = dataCatalog
                                                .createDatatypeProperty(Constants.CATALOG_URI + "DataCatalog#hasMD5");
                                Resource orig22 = dataCatalog.getResource(
                                                Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT + "/"
                                                                + EncodingUtils.encode(
                                                                                inputBinding2.substring(inputBinding2
                                                                                                .lastIndexOf("/") + 1,
                                                                                                inputBinding2.length())
                                                                                                + "_" + currentMD5
                                                                                                + "_V1"));
                                ...
                                // RDFS LABEL EXPORTED for canonical instance
                                // this.DataProps(Constants.RDFS_LABEL,
                                // AbstractSuperClass.getLocalName().substring(0,AbstractSuperClass.getLocalName().length()-5).toUpperCase()+finalversionforNewAbstractComponent,
                                // AbstractSuperClass.getLocalName().substring(0,AbstractSuperClass.getLocalName().length()-5),
                                // XSDDatatype.XSDstring);
                                for (String x : hsprops.keySet()) {
                                        OntProperty propSelec23;
                                        System.out.println("x " + x);
                                        propSelec23 = dataCatalog.createDatatypeProperty(
                                                        Constants.CATALOG_URI + "DataCatalog#" + x);
                                        Resource orig23 = dataCatalog.getResource(
                                                        Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT
                                                                        + "/"
                                                                        + EncodingUtils.encode(
                                                                                        inputBinding2.substring(
                                                                                                        inputBinding2.lastIndexOf(
                                                                                                                        "/")
                                                                                                                        + 1,
                                                                                                        inputBinding2.length())
                                                                                                        + "_"
                                        ...
                                        Resource orig2233 = dataCatalog
                                                        .getResource(Constants.CATALOG_URI + "DataCatalog#" + x);
                                ...
                                for (String x : similarNamesofInputFiles) {
                                        ...
                                        if (currenttemparr.size() == givenhspropsarr.size()
                                                        && currenttemparr.equals(givenhspropsarr))
                                                same = true;
                                ...
                                if (!same) {
                                        ...
                                        // the props are different somewhere so we find the latest version and link it
                                        // to it
                                        String finalversionforNewAbstractComponent = "";
                                        String finalversionforLatestAbstractComponent = "";
                                        int max = Integer.MIN_VALUE;
                                        ...
                                        System.out.println(
                                                        "finalversionforLatestAbstractComponent "
                                                                        + finalversionforLatestAbstractComponent);
                                        System.out.println("finalversionforNewAbstractComponent "
                                                        + finalversionforNewAbstractComponent);

                                        hmapforInputstoVersions.put(input2,
                                                        inputBinding2.substring(inputBinding2.lastIndexOf("/") + 1,
                                                                        inputBinding2.length())
                                                                        .toUpperCase() + "_" + currentMD5
                                                                        + finalversionforNewAbstractComponent);

                                        System.out.println(
                                                        "create a new version " + finalversionforNewAbstractComponent);
                                        ...
                                        String nameOfIndividualEnc = EncodingUtils
                                                        .encode(inputBinding2
                                                                        ...
                                                                        .toUpperCase() + "_" + currentMD5
                                                                        + finalversionforNewAbstractComponent);
                                        OntClass c = dataCatalog.createClass(
                                                        Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT);
                                        c.createIndividual(
                                                        Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT + "/"
                                                                        + nameOfIndividualEnc.toUpperCase());
                                        ...
                                        for (String x : hsprops.keySet()) {
                                                ...
                                                propSelec23 = dataCatalog.createDatatypeProperty(
                                                                Constants.CATALOG_URI + "DataCatalog#" + x);
                                                Resource orig23 = dataCatalog
                                                                .getResource(Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT
                                                                                + "/"
                                                                                + EncodingUtils.encode(inputBinding2
                                                                                                .substring(inputBinding2
                                                                                                                .lastIndexOf("/")
                                                                                                                + 1,
                                                                                                                inputBinding2.length())
                                                                                                + "_" + currentMD5
                                                ...
                                                OntProperty propSelec2233;
                                                propSelec2233 = dataCatalog
                                                                .createDatatypeProperty(Constants.RDFS_LABEL);
                                                Resource orig2233 = dataCatalog.getResource(
                                                                Constants.CATALOG_URI + "DataCatalog#" + x);
                                                dataCatalog.add(orig2233, propSelec2233, x);
                                        }

                                        // EXPORTING THE PROV_WAS_REVISION_OF
                                        OntProperty propSelec255 = dataCatalog
                                                        .createOntProperty(Constants.PROV_WAS_REVISION_OF);
                                        Resource source255 = dataCatalog
                                                        .getResource(Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT
                                                                        + "/"
                                                                        + EncodingUtils.encode(inputBinding2.substring(
                                                                                        inputBinding2.lastIndexOf("/")
                                                                                                        + 1,
                                                                                        inputBinding2.length()) + "_"
                                                                                        + currentMD5
                                                                                        + finalversionforNewAbstractComponent));
                ...
                while (r.hasNext()) {
                        ...
                        System.out.println("Step: " + step + " used input " + input + " with data binding: "
                                        + inputBinding);

                        // no need to add the variable individual now because the types are going to be
                        // added later
                        if (hmapforInputstoVersions.containsKey(input)) {
                                ModelUtils.addProperty(OPMWModel,
                                                Constants.CONCEPT_WORKFLOW_EXECUTION_PROCESS + "/" + step + date,
                                                Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT + "/"
                                                                + hmapforInputstoVersions.get(input),
                ...
                while (r.hasNext()) {
                        ...
                        ModelUtils.addIndividual(OPMWModel, paramName + date,
                                        ...
                                        "Parameter with value: " + paramvalue);
                        String auxParam = EncodingUtils
                                        .encode(Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT + "/" + paramName + date);
                        ...
                        if (res != null) {
                                ModelUtils.addProperty(OPMWModel,
                                                Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT + "/" + paramName + date,
                                                Constants.CONCEPT_PARAMETER_VARIABLE + "/" + expanded + templateToLink
                                                                + "_" + derived,
                ...
                while (r.hasNext()) {
                        ...
                        System.out.println("Step: " + step + " has output " + output + " with data binding: "
                                        + outputBinding);
                        ModelUtils.addProperty(OPMWModel,
                                        Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT + "/" + output + date,
                                        Constants.CONCEPT_WORKFLOW_EXECUTION_PROCESS + "/" + step + date,
                ...
                while (r.hasNext()) {
                        ...
                        if (hmapforInputstoVersions.containsKey(var)) {
                                auxP = EncodingUtils
                                                .encode(Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT + "/"
                                                                + hmapforInputstoVersions.get(var));
                        ...
                        if (prop.contains("derivedFrom")) {
                                ...
                                if (hmapforInputstoVersions.containsKey(var)) {
                                        ModelUtils.addProperty(OPMWModel,
                                                        Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT + "/"
                                                                        + hmapforInputstoVersions.get(var),
                                                        Constants.CONCEPT_DATA_VARIABLE + "/" + expanded
                                                                        + templateToLink + "_" + objName,
                                                        Constants.OPMW_PROP_CORRESPONDS_TO_TEMPLATE_ARTIFACT);
                                        // p-plan interop
                                        ModelUtils.addProperty(OPMWModel,
                                                        Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT + "/"
                                        ...
                                        ModelUtils.addProperty(OPMWModel,
                                                        Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT + "/" + var
                                                                        + date,
                                                        Constants.CONCEPT_DATA_VARIABLE + "/" + expanded
                                                                        + templateToLink + "_" + objName,
                                                        Constants.OPMW_PROP_CORRESPONDS_TO_TEMPLATE_ARTIFACT);
                        ...
                        // }
                                ...
                                if (hmapforInputstoVersions.containsKey(var)) {
                                        ...
                                        propSelec23 = OPMWModel.createDatatypeProperty(
                                                        Constants.CATALOG_URI + "DataCatalog#" + prop);
                                        Resource orig23 = OPMWModel
                                                        .getResource("http://www.opmw.org/export/resource/WorkflowExecutionArtifact/"
                                                                        + EncodingUtils.encode(hmapforInputstoVersions
                                                                                        .get(var)));
        ...
        private void dataProps(String dataprop, String resourcepart, String propextracted, XSDDatatype x) {
                ...
                Resource orig22 = taxonomyExport
                                .getResource(NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#"
                                                + EncodingUtils.encode(resourcepart));
        ...
        private void classIsaClass(String classpart, String indvpart) {
                OntClass c21 = taxonomyExport
                                .createClass(NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#" + classpart);
        ...
        private void inputsOutputs(String prop, HashSet<String> hs, String encodepart) {
                ...
                Resource source23 = taxonomyExport
                                .getResource(NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#"
                                                + EncodingUtils.encode(encodepart));
                Individual instance23 = (Individual) source23.as(Individual.class);
                for (String in : hs) {
                        if ((in).contains("http://")) {// it is a URI
                                instance23.addProperty(propSelec23,
                                                NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#" + in);
                        } else {// it is a local resource
                                instance23.addProperty(propSelec23, taxonomyExport
                                                .getResource(NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#"
                                                                + EncodingUtils.encode(in)));
                        }
                }
        }

        // function to check to export the expanded template or not. This should be an
        // ASK query
        ...
        public String createExpandedTemplate(String accname, String expandedTemplateName, String expandedTemplateURI,
                        String templateName, String domainName, String componentCatalogPath,
                        String componentDirectory) throws IOException {
                HashMap<String, String> classNames = new HashMap<>();
                NEW_TAXONOMY_CLASS_2 = Constants.CATALOG_URI + domainName + "/";
                // creating a new EXPORT NAME FOR THE TAXONOMY CLASS
                ...
                String newExpandedTemplateName = HashCreator.getExpandedTemplateHash(expandedTemplateName,
                                this.WINGSExecutionResults);

                // capturing the relationship between the execution account and the expanded
                // template
                ModelUtils.addProperty(OPMWModel, accname,
                ...
                // Constants.OPMW_PROP_IS_IMPLEMENTATION_OF_TEMPLATE);
                //

                // CAPTURING THE EXPANDED TEMPLATE AS A GRAPH, CAPTURING THE VERSION NUMBER, AND
                // DATA PROP FOR NATIVE SYSTEM TEMPLATE
                // ADDED A PROPERTY FOR CONNECTING THE EXECUTION ACCOUNT TO THE EXPANDED
                // TEMPLATE AND THE EXPANDED TEMPLATE TO THE TEMPLATE
                System.out.println("--------------------------");
                System.out.println("--------------------------");
                ...
                if (r1.hasNext()) {// there should be just one local name per template
                        ...
                        ModelUtils.addIndividual(OPMWModel, newExpandedTemplateName,
                                        Constants.OPMW_WORKFLOW_EXPANDED_TEMPLATE,
                                        "Workflow Expanded Template: " + newExpandedTemplateName);

                        // P-PLAN FOR EXPANDED TEMPLATE
                        OntClass cParam = OPMWModel.createClass(Constants.P_PLAN_PLAN);
                        cParam.createIndividual(
                                        Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_WORKFLOW_EXPANDED_TEMPLATE
                                                        + "/" + EncodingUtils.encode(newExpandedTemplateName));

                        if (v != null) {
                                ModelUtils.addDataProperty(OPMWModel,
                                                Constants.CONCEPT_WORKFLOW_EXPANDED_TEMPLATE + "/"
                                                                + newExpandedTemplateName,
                                                "" + v.getInt(),
                                                Constants.OPMW_DATA_PROP_VERSION_NUMBER, XSDDatatype.XSDint);
                        }
                        // add the uri of the original log file (native system template)
                        ModelUtils.addDataProperty(OPMWModel,
                                        Constants.CONCEPT_WORKFLOW_EXPANDED_TEMPLATE + "/" + newExpandedTemplateName,
                                        expandedTemplateURI, Constants.OPMW_DATA_PROP_HAS_NATIVE_SYSTEM_TEMPLATE,
                                        XSDDatatype.XSDanyURI);
                ...
                // MAINTAINING A COMPLETE AND FINAL DOMAIN NAME TO BE UTILIZED WHEN ITS GOING TO
                // BE EXPORTED WITH THE TAXONOMY MODEL
                while (r1.hasNext()) {
                        QuerySolution qs = r1.next();
                        Resource res = qs.getResource("?n");
                        Resource comp = qs.getResource("?c");
                        ...
                        // we have to
                        // 1.Query the component catalog to find the corresponding component exists or
                        // not
                        // 2. if it exists conintue
                        // 3.else we have to export it with the correct versioning code from the
                        // concrete component side
                        ...
                        String componentCatalogQueryforActualInputsandOutputsforComponent = Queries
                                        .componentCatalogQueryforActualInputsandOutputsforComponent(PREFIX_COMP_CATALOG,
                                                        tobequeriedclass);
                        ...
                        while (ractualzone.hasNext()) {
                                ...
                                if (nodenew.getLocalName().equals(
                                                res.getLocalName().substring(0, res.getLocalName().length() - 4))) {
                                        inputsComp.add(input.getLocalName());
                                        outputsComp.add(output.getLocalName());
                                        if (doc != null)
                                                doc11 = doc.getString();
                        ...
                        // NOW QUERYING MY COMPONENT CATALOG FOR FINDING THE INPUTS AND OUTPUTS OF THE
                        // ABSTRACT COMPONENT

                        String taxonomyModelforAbstractComponent = Queries
                                        .TaxonomyExportQuerySpecificComponentQueryAbstract();
                        ResultSet rformycomponentcatalogabstract = null;
                        rformycomponentcatalogabstract = queryLocalTaxonomyModelRepository(
                                        taxonomyModelforAbstractComponent);
                        ...
                        while (rformycomponentcatalogabstract.hasNext()) {
                                ...
                                String internalforeachcomp = Queries
                                                .TaxonomyExportQuerySpecificComponentQueryAbstract();
                                ...
                                while (internal1.hasNext()) {
                                        ...
                                        if (!c.getBoolean()) {
                                                if (nodenew.getLocalName().equals(nodenew1.getLocalName()) && nodenew
                                                                .getLocalName()
                                                                .toLowerCase()
                                                                .substring(0, nodenew.getLocalName().toLowerCase()
                                                                                .lastIndexOf("_"))
                                                                .equals(res2.getLocalName()
                                                                                .substring(0, res2.getLocalName()
                                                                                                .lastIndexOf("_"))
                                                                                .toLowerCase())) {
                                ...
                                if (inputsComp.size() == inpsfrommyac.size()
                                                && outpsfrommyac.size() == outputsComp.size())
                                        break;
                        }
                        System.out.println("The final abstract component to be linked to is:"
                                        + finalabstractcomponenttobelinkedto);

                        //// ENDS

                        // NOW QUERY MY COMPONENT CATALOG TO CHECK IF THIS COMPONENT WITH SAME INPS AND
                        // OUTPS EXISTS
                        // IF IT DOES UR DONE
                        ...
                        while (rformycomponentcatalog.hasNext()) {
                                ...
                                while (inter.hasNext()) {
                                        ...
                                        if (c.getBoolean() == true) {
                                                if (nodenew.getLocalName().equals(nodenew1.getLocalName()) && nodenew
                                                                ...
                                                                .substring(0, nodenew.getLocalName().toLowerCase()
                                                                                .lastIndexOf("_"))
                                                                .equals(res.getLocalName().substring(0,
                                                                                res.getLocalName().length() - 4)
                                                                                .toLowerCase())) {
                                ...
                                if (namefoundornot != null && inputsComp.size() == inpsfrommycc.size()
                                                && outputsComp.size() == outpsfrommycc.size()) {
                                        // now the name is found and plus the inputs and outputs are also equal and so
                                        // ur done here

                                        classNames.put(nodenew.getLocalName().substring(0,
                                                        nodenew.getLocalName().lastIndexOf("_")),
                                                        nodenew.getLocalName().substring(0,
                                                                        nodenew.getLocalName().lastIndexOf("_") + 1)
                                                                        + "CLASS"
                                                                        + nodenew.getLocalName().substring(nodenew
                                                                                        .getLocalName().lastIndexOf("_")
                                                                                        + 1));
                        ...
                        if (flag == 1) {
                                System.out.println(
                                                "U have found a match and hence u don't need to export anything from the expanded template zone end");
                                ...
                                try {
                                        ...
                                        this.dataProps(Constants.COMPONENT_HAS_MD5_CODE,
                                                        res.getLocalName().substring(0, res.getLocalName().length() - 4)
                                                                        .toUpperCase() + "_V1",
                                                        EncodingUtils.MD5ComponentCode(componentDirectory
                                                                        + compLoc.substring(
                                                                                        compLoc.lastIndexOf("/") + 1,
                                                                                        compLoc.length())
                                                                        + ".zip"),
                                                        XSDDatatype.XSDstring);
                                } catch (Exception e) {
                                ...
                                ModelUtils.addIndividualConcreteSubclass2(this.taxonomyExport,
                                                res.getLocalName().substring(0, res.getLocalName().length() - 4)
                                                                .toUpperCase() + "_CLASSV1",
                                                finalabstractcomponenttobelinkedto.substring(0,
                                                                finalabstractcomponenttobelinkedto.lastIndexOf("_") + 1)
                                                                + "CLASS"
                                                                + finalabstractcomponenttobelinkedto.substring(
                                                                                finalabstractcomponenttobelinkedto
                                                                                                .lastIndexOf("_") + 1,
                                                                                finalabstractcomponenttobelinkedto
                                                                                                .length()));
                                ...
                                this.classIsaClass(
                                                ...
                                                res.getLocalName().substring(0, res.getLocalName().length() - 4)
                                                                .toUpperCase() + "_V1");
                                // classNames.put(res.getLocalName().substring(0,res.getLocalName().length()-4).toUpperCase(),
                                // res.getLocalName().substring(0,res.getLocalName().length()-4).toUpperCase()+"_CLASSV1");
                                // EXPORTING THE DOCUMENTATION
                                if (doc11 != null) {
                                        this.dataProps(Constants.COMPONENT_HAS_DOCUMENTATION,
                                                        res.getLocalName().substring(0, res.getLocalName().length() - 4)
                                ...
                                classNames.put(res.getLocalName().substring(0, res.getLocalName().length() - 4)
                                                .toUpperCase(),
                                                res.getLocalName().substring(0, res.getLocalName().length() - 4)
                                                                .toUpperCase() + "_CLASSV1");
                                ...
                                this.dataProps(Constants.COMPONENT_HAS_LOCATION,
                                                res.getLocalName().substring(0, res.getLocalName().length() - 4)
                                                                .toUpperCase() + "_V1",
                                                componentDirectory + compLoc.substring(compLoc.lastIndexOf("/") + 1,
                                                                compLoc.length()),
                                                XSDDatatype.XSDstring);

                                // now the only thing left to be done at this stage for a fresh new export is to
                                // just link it with the appropriate
                                // abstract component which will already be present in my component catalog

                        } else if (namefoundornot != null) {
                                // now the thing is that a name has been found but its not having the same
                                ...
                                // now simply export using this version number and link to previous version
                                // number
                                ...
                                this.dataProps2(Constants.OWL_VERSION_INFO,
                                                res.getLocalName().substring(0, res.getLocalName().length() - 4)
                                                                .toUpperCase() + "_CLASSV"
                                                                + versionnumber,
                                                versionnumber + "", XSDDatatype.XSDstring);
                                ...
                                OntProperty propSelec255 = taxonomyExport
                                                .createOntProperty(Constants.PROV_WAS_REVISION_OF);
                                Resource source255 = taxonomyExport.getResource(NEW_TAXONOMY_CLASS.replace("#", "")
                                                + "/Component#"
                                                + EncodingUtils
                                                                .encode(res.getLocalName().substring(0,
                                                                                res.getLocalName().length() - 4)
                                                                                .toUpperCase()
                                                                                + "_V" + versionnumber));
                                Individual instance255 = (Individual) source255.as(Individual.class);
                                if ((res.getLocalName().toUpperCase() + "_V" + max).contains("http://")) {// it is a URI
                                        instance255.addProperty(propSelec255,
                                                        NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#"
                                                                        + res.getLocalName().substring(0,
                                                                                        res.getLocalName().length() - 4)
                                                                                        .toUpperCase()
                                                                        + "_V" + max);
                                } else {// it is a local resource
                                        instance255.addProperty(propSelec255,
                                                        taxonomyExport.getResource(NEW_TAXONOMY_CLASS.replace("#", "")
                                                                        + "/Component#"
                                                                        + EncodingUtils.encode(
                                                                                        res.getLocalName().substring(0,
                                                                                                        res.getLocalName()
                        ...
                        System.out.println("node is :" + res.getLocalName() + " derived from " + res2.getLocalName()
                                        + "the component is :" + comp);
                        System.out.println("cb is :" + typeComp);
                        System.out.println("this is inside the node linking new expanded");

                        // this.addIndividual(OPMWModel,templateName_+res.getLocalName(),Constants.OPMW_WORKFLOW_TEMPLATE_PROCESS,
                        // "Workflow template process "+res.getLocalName());
                        // CURRENTLY I AM COMMENTING THIS TO AVOID EXTRA STUFF BEING EXPORTED
                        ModelUtils.addIndividual(OPMWModel,
                                        "Expanded_" + newExpandedTemplateName + "_" + res.getLocalName(),
                                        Constants.OPMW_WORKFLOW_TEMPLATE_PROCESS,
                                        "Workflow expanded template process " + res.getLocalName());

                        // p-plan interop
                        OntClass cStep = OPMWModel.createClass(Constants.P_PLAN_STEP);
                        cStep.createIndividual(Constants.PREFIX_EXPORT_RESOURCE
                                        + Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/"
                                        + EncodingUtils.encode("Expanded_" + newExpandedTemplateName + "_"
                                                        + res.getLocalName()));

                        if (typeComp.isURIResource()) { // only adds the type if the type is a uRI (not a blank node)
                                // String tempURI =
                                // EncodingUtils.encode(Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS+"/"+"Expanded_"+newExpandedTemplateName+"_"+res.getLocalName());
                                // OntClass cAux1 = OPMWModel.createClass(typeComp.getURI());//repeated tuples
                                ...
                                String newchangeforthetype2 = newchangeforthetype.substring(
                                                newchangeforthetype.lastIndexOf("#"),
                                                newchangeforthetype.length());
                                String tempURI = EncodingUtils
                                                .encode(Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + "Expanded_"
                                                                + newExpandedTemplateName + "_" + res.getLocalName());
                                // OntClass cAux1 =
                                // OPMWModel.createClass(NEW_TAXONOMY_CLASS_2+"Component"+newchangeforthetype2);//repeated
                                // tuples will not be duplicated
                        ...
                        if (rule != null) {
                                // rules are strings
                                ModelUtils.addDataProperty(OPMWModel,
                                                Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + "Expanded_"
                                                                + newExpandedTemplateName + "_"
                                                                + res.getLocalName(),
                                                rule.getString(),
                                                Constants.WINGS_DATA_PROP_HAS_RULE);
                        }
                        ...
                        // ModelUtils.addProperty(OPMWModel,
                        // Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS+"/"+"Expanded_"+newExpandedTemplateName+"_"+res.getLocalName(),
                        // Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS+"/"+res2.getLocalName(),
                        // Constants.OPMW_PROP_IS_IMPLEMENTATION_OF_TEMPLATE_PROCESS);
                        ...
                        ModelUtils.addProperty(OPMWModel,
                                        Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + "Expanded_"
                                                        + newExpandedTemplateName + "_"
                                                        + res.getLocalName(),
                                        Constants.CONCEPT_WORKFLOW_TEMPLATE + "/" + "Expanded_"
                                                        + newExpandedTemplateName,
                                        Constants.P_PLAN_PROP_IS_STEP_OF_PLAN);
                ...
                while (r1.hasNext()) {
                        ...
                        ModelUtils.addIndividual(OPMWModel,
                                        "Expanded_" + newExpandedTemplateName + "_" + variable.getLocalName(),
                                        Constants.OPMW_DATA_VARIABLE, "Data variable " + variable.getLocalName());
                        ...
                        String aux = EncodingUtils.encode(Constants.CONCEPT_DATA_VARIABLE + "/" + "Expanded_"
                                        + newExpandedTemplateName + "_" + variable.getLocalName());
                        OntClass cAux1 = OPMWModel.createClass(Constants.OPMW_WORKFLOW_TEMPLATE_ARTIFACT);// repeated
                                                                                                          // tuples will
                                                                                                          // not be
                                                                                                          // duplicated
                        ...
                        if (type != null) {
                                ...
                                if (type.isURIResource()) {
                                        ...
                                        // String nameEncoded =
                                        // EncodingUtils.encode(Constants.CONCEPT_DATA_VARIABLE+"/"+"Expanded_"+newExpandedTemplateName+"_"+variable.getLocalName());
                                        // OntClass c = OPMWModel.createClass(type.getURI());
                                        // c.createIndividual(Constants.PREFIX_EXPORT_RESOURCE+nameEncoded);

                                        // String
                                        // newchangeforthetype=type.getURI().substring(type.getURI().lastIndexOf("/"),type.getURI().length());
                                        // String nameEncoded =
                                        // EncodingUtils.encode(Constants.CONCEPT_DATA_VARIABLE+"/"+"Expanded_"+newExpandedTemplateName+"_"+variable.getLocalName());
                                        // OntClass c =
                                        // OPMWModel.createClass(NEW_TAXONOMY_CLASS_2+"Data"+newchangeforthetype);
                                        // c.createIndividual(Constants.PREFIX_EXPORT_RESOURCE+nameEncoded);
                                } else {
                                        System.out.println("ANON RESOURCE " + type.getURI() + " ignored");
                                }
                        ...
                        // this.addDataProperty(OPMWModel,
                        // Constants.CONCEPT_DATA_VARIABLE+"/"+"Expanded_"+expandedTemplateNewName,""+
                        // databinding,
                        // Constants.OPMW_PROP_IS_DATA_BINDING_OF_EXPANDED_TEMPLATE_DATA_VARIABLE,XSDDatatype.XSDanyURI);

                        // is variable of expanded template
                        ModelUtils.addProperty(OPMWModel,
                                        Constants.CONCEPT_DATA_VARIABLE + "/" + "Expanded_" + newExpandedTemplateName
                                                        + "_"
                                                        + variable.getLocalName(),
                                        Constants.CONCEPT_WORKFLOW_EXPANDED_TEMPLATE + "/" + newExpandedTemplateName,
                                        Constants.OPMW_PROP_IS_VARIABLE_OF_TEMPLATE);
                ...
                while (r1.hasNext()) {
                        ...
                        String aux = EncodingUtils.encode(Constants.CONCEPT_PARAMETER_VARIABLE + "/" + "Expanded_"
                                        + newExpandedTemplateName + "_" + res.getLocalName());
                        OntClass cAux1 = OPMWModel.createClass(Constants.OPMW_WORKFLOW_TEMPLATE_ARTIFACT);// repeated
                                                                                                          // tuples will
                                                                                                          // not be
                        ...
                        ModelUtils.addProperty(OPMWModel,
                                        Constants.CONCEPT_PARAMETER_VARIABLE + "/" + "Expanded_"
                                                        + newExpandedTemplateName + "_"
                                                        + res.getLocalName(),
                                        Constants.CONCEPT_WORKFLOW_EXPANDED_TEMPLATE + "/" + newExpandedTemplateName,
                                        Constants.OPMW_PROP_IS_PARAMETER_OF_TEMPLATE);
                        ...
                        ModelUtils.addProperty(OPMWModel,
                                        Constants.CONCEPT_PARAMETER_VARIABLE + "/" + "Expanded_"
                                                        + newExpandedTemplateName + "_"
                                                        + res.getLocalName(),
                                        Constants.CONCEPT_PARAMETER_VARIABLE + "/" + templateName + "/"
                                                        + derivedFrom.getLocalName(),
                                        Constants.OPMW_PROP_IS_IMPLEMENTATION_OF_TEMPLATE_PARAMETER_VARIABLE);
                        ...
                        ModelUtils.addDataProperty(OPMWModel,
                                        Constants.CONCEPT_PARAMETER_VARIABLE + "/" + "Expanded_"
                                                        ...
                                                        + res.getLocalName(),
                                        parValue + "",
                                        Constants.OPMW_PROP_IS_PARVALUE_OF_EXPANDED_TEMPLATE_PARAMETER_VARIABLE);
                        ...
                        if (role != null) {
                                ...
                                ModelUtils.addProperty(OPMWModel,
                                                Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + "Expanded_"
                                                                + newExpandedTemplateName + "_"
                                                                + resNode.getLocalName(),
                                                Constants.CONCEPT_DATA_VARIABLE + "/" + "Expanded_"
                                                                + newExpandedTemplateName + "_"
                                                                + resVar.getLocalName(),
                                                Constants.PREFIX_EXTENSION + "usesAs_" + role);
                                // link the property as a subproperty of Used
        ...
        private void exportHardwareSoftwareDependencies(boolean needsval, Float memval, Float stval,
                        Float minversionval,
                        Resource concrComponent, int version_no) {
                Resource blankNode112 = taxonomyExport
                                .createResource(Constants.PREFIX_RESOURCE + "HardwareRequirements_"
                                                + concrComponent.getLocalName().substring(0,
                                                                concrComponent.getLocalName().length() - 5)
                                                + "_V"
                                                + version_no);
                blankNode112.addProperty(taxonomyExport.createOntProperty(Constants.WINGS_PROP_NEEDS_64BIT),
                                needsval + "", XSDDatatype.XSDboolean);
                if (memval != null) {
                        blankNode112.addProperty(
                                        taxonomyExport.createOntProperty(Constants.WINGS_PROP_REQUIRES_MEMORYGB),
                ...
                String procURI = NEW_TAXONOMY_CLASS.replace("#", "/Component#") + EncodingUtils
                                .encode(concrComponent.getLocalName().substring(0,
                                                concrComponent.getLocalName().length() - 5) + "_V"
                                                + version_no);
                taxonomyExport.getResource(procURI).addProperty(
                                taxonomyExport.createOntProperty(Constants.WINGS_PROP_HAS_HARDWARE_DEPENDENCY),
                                blankNode112);
                this.classIsaClassHardwareParts(Constants.HARDWARE_DEPENDENCY,
                                Constants.PREFIX_RESOURCE + "HardwareRequirements_"
                                                + concrComponent.getLocalName().substring(0,
                                                                concrComponent.getLocalName().length() - 5)
                                                + "_V"
                                                + version_no);
                Resource blankNode113 = taxonomyExport
                                .createResource(Constants.PREFIX_RESOURCE + "SoftwareRequirements_"
                                                + concrComponent.getLocalName().substring(0,
                                                                concrComponent.getLocalName().length() - 5)
                                                + "_V"
                                                + version_no);
                ...
                String procURI113 = NEW_TAXONOMY_CLASS.replace("#", "/Component#") + EncodingUtils
                                .encode(concrComponent.getLocalName().substring(0,
                                                concrComponent.getLocalName().length() - 5) + "_V"
                                                + version_no);
                taxonomyExport.getResource(procURI113).addProperty(
                                taxonomyExport.createOntProperty(Constants.WINGS_PROP_HAS_SOFTWARE_DEPENDENCY),
                                blankNode113);
                this.classIsaClassHardwareParts(Constants.SOFTWARE_DEPENDENCY,
                                Constants.PREFIX_RESOURCE + "SoftwareRequirements_"
                                                + concrComponent.getLocalName().substring(0,
                                                                concrComponent.getLocalName().length() - 5)
                                                + "_V"
                                                + version_no);
