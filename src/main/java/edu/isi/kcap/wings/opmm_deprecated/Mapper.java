...
public class Mapper {
    ...
    public String transformWINGSElaboratedTemplateToOPMW(String wingsInputTemplatePath, String inputMode,
        ...
        if (r.hasNext()) {// there should be just one local name per template
            ...
            cParam.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_WORKFLOW_TEMPLATE + "/"
            ...
            plan.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_WORKFLOW_TEMPLATE + "/"
                    + EncodingUtils.encode(newTemplateName));
        ...
        while (r.hasNext()) {
            ...
            if (contrib != null) {
                ...
                ModelUtils.addProperty(OPMWModel, Constants.CONCEPT_WORKFLOW_TEMPLATE + "/" + newTemplateName,
            ...
            while (rnew1.hasNext()) {
                ...
                if (x != null) {
                    ...
                    if (nodenew.getLocalName().equals(className) && concr != null) {
                        ...
                        System.out.println("y is " + y.getLocalName() + " n is " + nodenew.getLocalName() + " x is "
            ...
            if (concreteness != null && concreteness.equals("false")) {
                if (!AbstractSuperClass.getLocalName().equals("Component"))
                    specialCaseforFolders.add(
                            concrComponent.getLocalName().substring(0, concrComponent.getLocalName().length() - 5));
        ...
        while (r.hasNext()) {
            ...
            System.out.println(res + " Node has component " + comp + " of type: " + typeComp);// + " which is concrete:
            ...
            if (!hs1Concr.contains(className.substring(0, className.length() - 5))
                    && !hs2Abs.contains(className.substring(0, className.length() - 5)) && !hs1Concr.contains(className)
            ...
            ModelUtils.addIndividual(OPMWModel, newTemplateName_ + res.getLocalName(),
                    Constants.OPMW_WORKFLOW_TEMPLATE_PROCESS, "Workflow template process " + res.getLocalName());
            // p-plan interop
            OntClass cStep = OPMWModel.createClass(Constants.P_PLAN_STEP);
            cStep.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/"
            ...
            if (!hsforComps.contains(className)) {
                ...
                if (deciderPoint == 1) {
                    ...
                    String componentCatalogQueryforInputsandOutputsComponent = Queries
                            .componentCatalogQueryforInputsandOutputs(PREFIX_COMP_CATALOG, className);
                    ...
                    if (hs2Abs.contains(className)
                            || hs2Abs.contains(className.substring(0, className.length() - 5)) && nodenew11 != null) {
                        boolean specialFolder = false;
                        if (specialCaseforFolders.contains(className)
                                || specialCaseforFolders.contains(className.substring(0, className.length() - 5))) {
                        ...
                        while (r2new.hasNext()) {
                            ...
                            if (!isComponentConcrete) {
                                ...
                                if (nodenew.getLocalName().toLowerCase()
                                        .contains(nodenew11.getLocalName().toLowerCase())) {
                        ...
                        if (namesOfAbstractCompswithSameNames.isEmpty()) {
                            ...
                            // start exporting the inputs and outputs for the abstract component and
                            // canonical instance

                            // extracting the actual inputs,outputs and other factors for the Abstract
                            // component
                            ...
                            String componentCatalogQueryforActualInputsandOutputsforAbstractComponent = Queries
                                    .componentCatalogQueryforActualInputsandOutputsforAbstractComponent(
                                            PREFIX_COMP_CATALOG, nodenew11.getLocalName() + "Class");
                            rnew2 = null;
                            ...
                            this.dataProps(Constants.PROV_WAS_ATTRIBUTED_TO,
                                    nodenew11.getLocalName().toUpperCase() + "_V1", userName, XSDDatatype.XSDstring);

                            // EXPORTING THE FACT THAT CLASSNAME-CLASS IS A CLASSNAME
                            this.classIsaClass(nodenew11.getLocalName().toUpperCase() + "_CLASSV1",
                                    nodenew11.getLocalName().toUpperCase() + "_V1");
                            classNames.put(nodenew11.getLocalName().toUpperCase(),
                                    nodenew11.getLocalName().toUpperCase() + "_CLASSV1");

                            // IS CONCRETE EXPORTED
                            this.dataProps(Constants.COMPONENT_IS_CONCRETE,
                                    nodenew11.getLocalName().toUpperCase() + "_V1", compConcreteAbs + "",
                                    XSDDatatype.XSDboolean);

                            // RDFS LABEL EXPORTED for canonical instance
                            this.dataProps(Constants.RDFS_LABEL, nodenew11.getLocalName().toUpperCase() + "_V1",
                                    nodenew11.getLocalName(), XSDDatatype.XSDstring);

                            // RDFS LABEL EXPORTED for class
                            this.dataProps2(Constants.RDFS_LABEL, nodenew11.getLocalName().toUpperCase() + "_CLASSV1",
                                    nodenew11.getLocalName(), XSDDatatype.XSDstring);
                            this.dataProps2(Constants.OWL_VERSION_INFO,
                                    nodenew11.getLocalName().toUpperCase() + "_CLASSV1", "1", XSDDatatype.XSDstring);
                            ...
                            // OUTPUTS-- EXPORTED
                            this.inputsOutputs(Constants.COMPONENT_HAS_OUTPUT, outputsAbsComp,
                                    nodenew11.getLocalName().toUpperCase() + "_V1");
                            ...
                            if (specialFolder == true) {
                                ...
                                this.dataProps("http://www.w3.org/2000/01/rdf-schema#subClassOf",
                                        nodenew11.getLocalName().toUpperCase() + "_V1",
                                        AbstractSuperClass.getLocalName(), XSDDatatype.XSDstring);
                            }
                            ...
                            // STEP5: EXTRACT ONLY THE (INPUTS) DATA VARIABLES IF THEY EXIST FOR ABSTRACT
                            // COMPONENTS
                            ...
                            // STEP6: EXTRACT ONLY THE (OUTPUTS) DATA VARIABLES IF THEY EXIST FOR ABSTRACT
                            // COMPONENTS
                        ...
                        // Case2: Checking the components once for same Inputs and Outputs
                            ...
                            // this means we have found a list of similar abstract components in the
                            // Taxonomy Model
                            // now there are 2 points we have to consider here
                            // first we have to check the inputs and outputs of all the similar named
                            // abstract components in the Taxonomy Model
                            // if there is one with the same number of inputs and outputs then just break
                            // off
                            // if there is no one with the same inputs and outputs then we have to export a
                            // new one out
                            ...
                            // NOW WE HAVE TO FIND OUT THE NUMBER OF INPUTS AND OUTPUTS IN EACH OF THE
                            ...
                            for (String whatwehave : namesOfAbstractCompswithSameNames) {
                                ...
                                if (similarAbsInps.size() == inputsAbsComp.size()
                                        && similarAbsOuts.size() == outputsAbsComp.size()) {

                                    // add to classNames the appropriate version of the class that we have to point
                                    // to.
                                    classNames.put(nodeinstanceans.substring(0, nodeinstanceans.lastIndexOf("_")),
                                            nodeclassans);
                            ...
                            // Case3: Export since there are no components with same number of inputs and
                            // outputs
                            if (clarifyToExportAbstractComp == true) {
                                // BIG TASK TO DO HERE IS TO IDENTIFY THE LATEST VERSION OF THE COMPONENT OUT OF
                                // ALL OTHER COMPONENTS
                                // SO THAT WHEN WE ARE CREATING THE NEW ABSTRACT COMPONENT
                                // WE WANT TO LINK THE LAST AND CURRENT ABSTRACT COMPONENTS WITH THE CANONICAL
                                // INSTANCES
                                ...
                                System.out
                                        .println("THE MOST LATEST VERSION IS : " + finalversionforNewAbstractComponent);
                                // ABOVE TASK DONE BY HERE

                                // this means that the same Abstract Component is not present. So now we will
                                // export a fresh one.
                                ...
                                Resource source255 = taxonomyExport
                                        .getResource(NEW_TAXONOMY_CLASS.replace("#", "/Component#")
                                                + EncodingUtils.encode(nodenew11.getLocalName().toUpperCase()
                                                        + finalversionforNewAbstractComponent));
                                Individual instance255 = (Individual) source255.as(Individual.class);
                                if ((nodenew11.getLocalName().toUpperCase() + finalversionforLatestAbstractComponent)
                                    ...
                                    instance255.addProperty(propSelec255,
                                            taxonomyExport.getResource(NEW_TAXONOMY_CLASS.replace("#", "/Component#")
                                                    + EncodingUtils.encode(nodenew11.getLocalName().toUpperCase()
                                                            + finalversionforLatestAbstractComponent)));
                                }
                                ...
                                // EXPORTING THE USER WHO CREATED THE COMPONENT
                                this.dataProps(Constants.PROV_WAS_ATTRIBUTED_TO,
                                        nodenew11.getLocalName().toUpperCase() + finalversionforNewAbstractComponent,
                                        userName, XSDDatatype.XSDstring);

                                // EXPORTING THE FACT THAT CLASSNAME-CLASS IS A CLASSNAME
                                this.classIsaClass(
                                        nodenew11.getLocalName().toUpperCase() + "_CLASS"
                                                + finalversionforNewAbstractComponent.substring(1,
                                                        finalversionforNewAbstractComponent.length()),
                                        nodenew11.getLocalName().toUpperCase() + finalversionforNewAbstractComponent);
                                classNames.put(nodenew11.getLocalName().toUpperCase(),
                                        nodenew11.getLocalName().toUpperCase() + "_CLASS"
                                                + finalversionforNewAbstractComponent.substring(1,
                                ...
                                this.dataProps2(Constants.RDFS_LABEL,
                                        nodenew11.getLocalName().toUpperCase() + "_CLASS"
                                                + finalversionforNewAbstractComponent.substring(1,
                                                        finalversionforNewAbstractComponent.length()),
                                        finalversionforNewAbstractComponent, XSDDatatype.XSDstring);
                    ...
                    ///// CONCRETE COMPONENT**********////
                            ...
                            || hs1Concr.contains(className.substring(0, className.length() - 5))) {
                        ...
                        while (r2new.hasNext()) {
                            ...
                            if (c.getBoolean()) {
                                if (nodenew.getLocalName().toLowerCase().contains(concrComponent.getLocalName()
                                        .substring(0, concrComponent.getLocalName().length() - 5).toLowerCase())) {
                                    // this means the name exists
                                    // next step is to check the inputs and outputs based on that className
                                    // saving all such cases in a HashSet
                                    if (!namesOfConcreteCompswithSameNames.contains(nodenew.getLocalName())
                                            && !nodenew.getLocalName().contains("CLASS")) {
                                        System.out.println("nodenew " + nodenew.getLocalName() + " x "
                                                + x.getLocalName() + " c " + c.getBoolean());
                        ...
                        while (r2new.hasNext()) {
                            ...
                            if (!concrete.getBoolean()) {
                                if (nodenew.getLocalName().toLowerCase().contains(AbstractSuperClass.getLocalName()
                                        .substring(0, AbstractSuperClass.getLocalName().length() - 5).toLowerCase())) {
                                    // this means the name exists
                                    // next step is to check the inputs and outputs based on that className
                                    // saving all such cases in a HashSet
                        ...
                        // IF THE SIZE OF THE HASHSET namesOfConcreteCompswithSameNames IS !=0 THEN ITS
                        ...
                        if (namesOfAbsCompswithSameNames.size() != 0 && namesOfConcreteCompswithSameNames.size() == 0) {
                            ...
                            if (clarifyToExportConcrComp == 1) {
                                ...
                                ModelUtils.addIndividualConcreteSubclass2(this.taxonomyExport,
                                        concrComponent.getLocalName()
                                                .substring(0, concrComponent.getLocalName().length() - 5).toUpperCase()
                                                + "_CLASSV1",
                                        Abs.getLocalName().substring(0, Abs.getLocalName().lastIndexOf("_")) + "_CLASS"
                                                + Abs.getLocalName().substring(Abs.getLocalName().lastIndexOf("_") + 1,
                                                        Abs.getLocalName().length()));

                                // for the component blankNode extraction for HARDWARE and SOFTWARE
                                // dependencies:
                                String componentCatalogQuerydependencies = Queries.componentCatalogQuerydependencies(
                                        PREFIX_COMP_CATALOG, concrComponent.getLocalName().substring(0,
                                                concrComponent.getLocalName().length() - 5) + "Class");
                                rnew2 = null;
                                ...
                                while (rnew2.hasNext()) {
                                    ...
                                    if (nodenew.getLocalName().equals(concrComponent.getLocalName().substring(0,
                                ...
                                try {
                                    this.dataProps(Constants.COMPONENT_HAS_MD5_CODE,
                                            concrComponent.getLocalName()
                                                    .substring(0, concrComponent.getLocalName().length() - 5)
                                                    .toUpperCase() + "_V1",
                                            EncodingUtils.MD5ComponentCode(componentDirectory
                                                    + compLoc.substring(compLoc.lastIndexOf("\\") + 1, compLoc.length())
                                                    + ".zip"),
                            ...
                            if (clarifyToExportConcrComp == 0) {
                                // this means we export both as a new version (LATEST VERSION) AND LINKING ALSO
                                // CANONICAL INSTANCES
                                ...
                                ModelUtils.addIndividualConcreteSubclass2(this.taxonomyExport,
                                        ...
                                        AbstractSuperClass.getLocalName().substring(0,
                                                AbstractSuperClass.getLocalName().length() - 5) + "_CLASS"
                                                + finalversionforNewAbstractComponent.substring(1,
                                                        finalversionforNewAbstractComponent.length()));
                                ...
                                Resource source255 = taxonomyExport
                                        .getResource(NEW_TAXONOMY_CLASS.replace("#", "/Component#")
                                                + EncodingUtils.encode(AbstractSuperClass.getLocalName()
                                                        .substring(0, AbstractSuperClass.getLocalName().length() - 5)
                                                        .toUpperCase() + finalversionforNewAbstractComponent));
                                Individual instance255 = (Individual) source255.as(Individual.class);
                                if ((AbstractSuperClass.getLocalName()
                                        .substring(0, AbstractSuperClass.getLocalName().length() - 5).toUpperCase()
                                        + finalversionforLatestAbstractComponent).contains("http://")) {// it is a URI
                                    instance255
                                            .addProperty(propSelec255,
                                                    NEW_TAXONOMY_CLASS.replace("#", "/Component#")
                                                            + AbstractSuperClass.getLocalName()
                                                                    .substring(0,
                                                                            AbstractSuperClass.getLocalName().length()
                                                                                    - 5)
                                                                    .toUpperCase()
                                                            + finalversionforLatestAbstractComponent);
                                } else {// it is a local resource
                                    instance255.addProperty(propSelec255, taxonomyExport.getResource(NEW_TAXONOMY_CLASS
                                            .replace("#", "/Component#")
                                            + EncodingUtils.encode(AbstractSuperClass.getLocalName()
                                                    .substring(0, AbstractSuperClass.getLocalName().length() - 5)
                                                    .toUpperCase() + finalversionforLatestAbstractComponent)));
                                }
                                ...
                                classNames.put(
                                        ...
                                        concrComponent.getLocalName()
                                                .substring(0, concrComponent.getLocalName().length() - 5).toUpperCase()
                                                + "_CLASS" + finalversionforNewAbstractComponent.substring(1,
                                                        finalversionforNewAbstractComponent.length()));
                                ...
                                // extracting the actual inputs,outputs and other factors for the Abstract
                                // component
                                System.out.println("EXTRACTION BEGINS " + AbstractSuperClass.getLocalName().substring(0,
                                        AbstractSuperClass.getLocalName().length() - 5));
                                String componentCatalogQueryforActualInputsandOutputsforAbstractComponent = Queries
                                        .componentCatalogQueryforActualInputsandOutputsforAbstractComponent(
                                                PREFIX_COMP_CATALOG, AbstractSuperClass.getLocalName().substring(0,
                                                        AbstractSuperClass.getLocalName().length() - 5) + "Class");
                                rnew2 = null;
                                rnew2 = queryComponentCatalog(
                                        componentCatalogQueryforActualInputsandOutputsforAbstractComponent);
                                ...
                                HashSet<String> outputsAbsComp = new HashSet<>();
                                boolean compConcreteAbs = false;
                                System.out.println("what we have " + AbstractSuperClass.getLocalName().substring(0,
                                        AbstractSuperClass.getLocalName().length() - 5));
                                while (rnew2.hasNext()) {
                                    QuerySolution qsnew = rnew2.next();
                                    Resource nodenew = qsnew.getResource("?n");
                                    ...
                                    if (nodenew.getLocalName().equals(AbstractSuperClass.getLocalName().substring(0,
                                            AbstractSuperClass.getLocalName().length() - 5))) {
                                        inputsAbsComp.add(input.getLocalName());
                                        outputsAbsComp.add(output.getLocalName());
                                        if (concr != null) {
                                ...
                                // RDFS LABEL EXPORTED for class
                                this.dataProps2(Constants.RDFS_LABEL, AbstractSuperClass.getLocalName()
                                        .substring(0, AbstractSuperClass.getLocalName().length() - 5).toUpperCase()
                                        + "_CLASS"
                                        + finalversionforNewAbstractComponent.substring(1,
                                                finalversionforNewAbstractComponent.length()),
                                        AbstractSuperClass.getLocalName().substring(0,
                                                AbstractSuperClass.getLocalName().length() - 5),
                                        XSDDatatype.XSDstring);
                                this.dataProps2(Constants.OWL_VERSION_INFO, AbstractSuperClass.getLocalName()
                                        .substring(0, AbstractSuperClass.getLocalName().length() - 5).toUpperCase()
                                        + "_CLASS"
                                        + finalversionforNewAbstractComponent.substring(1,
                                                finalversionforNewAbstractComponent.length()),
                        ...
                        // Case2: Direct Export since the size of the found concr comps with similar
                        // names is 0
                        if (namesOfConcreteCompswithSameNames.isEmpty() && namesOfAbsCompswithSameNames.isEmpty()) {

                            // EXPORTING IT NOW:
                            System.out.println("ULTIMATE POINT ::::CHECK::::" + concrComponent.getLocalName()
                                    .substring(0, concrComponent.getLocalName().length() - 5));
                            ...
                            ModelUtils.addIndividualConcreteSubclass2(this.taxonomyExport,
                                    concrComponent.getLocalName()
                                            ...
                                            + "_CLASSV1",
                                    AbstractSuperClass.getLocalName().substring(0,
                                            AbstractSuperClass.getLocalName().length() - 5) + "_CLASSV1");
                            ...
                            try {
                                System.out.println(componentDirectory
                                        + compLoc.substring(compLoc.lastIndexOf("/") + 1, compLoc.length()) + ".zip");
                                this.dataProps(Constants.COMPONENT_HAS_MD5_CODE,
                                        concrComponent.getLocalName()
                                                .substring(0, concrComponent.getLocalName().length() - 5).toUpperCase()
                                                + "_V1",
                            ...
                            String componentCatalogQueryforActualInputsandOutputsforAbstractComponent = Queries
                                    .componentCatalogQueryforActualInputsandOutputsforAbstractComponent(
                                            PREFIX_COMP_CATALOG, AbstractSuperClass.getLocalName());
                            rnew2 = null;
                            rnew2 = queryComponentCatalog(
                                    componentCatalogQueryforActualInputsandOutputsforAbstractComponent);
                            ...
                            // EXPORTING THE FACT THAT CLASSNAME-CLASS IS A CLASSNAME
                            this.classIsaClass(
                                    AbstractSuperClass.getLocalName().toUpperCase().substring(0,
                                            AbstractSuperClass.getLocalName().length() - 5) + "_CLASSV1",
                                    AbstractSuperClass.getLocalName()
                                            .substring(0, AbstractSuperClass.getLocalName().length() - 5).toUpperCase()
                                            + "_V1");
                        ...
                        // Case2: Checking the components once for same Inputs and Outputs
                            ...
                            boolean compConcrete = false;
                            String compLoc = null;
                            System.out.println("what we have " + concrComponent.getLocalName().substring(0,
                                    concrComponent.getLocalName().length() - 5));
                            while (rnew2.hasNext()) {
                                QuerySolution qsnew = rnew2.next();
                                Resource nodenew = qsnew.getResource("?n");
                            ...
                            for (String whatwehave : namesOfConcreteCompswithSameNames) {
                                ...
                                while (rnew2.hasNext()) {
                                    ...
                                    if (nodenew.getLocalName().toUpperCase().equals(whatwehave)
                                            && !nodenew.getLocalName().toUpperCase().equals(x.getLocalName())) {
                                        nodeInstanceans = nodeInstance;
                                        nodeClassans = nodeClass;
                                        System.out.println("Node inside case2:? " + nodenew.getLocalName());
                                        System.out.println("x inside case2: ? " + x.getLocalName());
                                ...
                                try {
                                    System.out.println(componentDirectory
                                            + compLoc.substring(compLoc.lastIndexOf("/") + 1, compLoc.length())
                                            + ".zip");
                                    one = EncodingUtils.MD5ComponentCode(componentDirectory
                                            + compLoc.substring(compLoc.lastIndexOf("/") + 1, compLoc.length())
                                            + ".zip");
                                } catch (Exception e) {
                                ...
                                if (similarAbsInps.size() == inputsCurrentConcrComp.size()
                                        && similarAbsOuts.size() == outputsCurrentConcrComp.size()) {
                                    ...
                                    // the else here means that the inputs and outputs are the same but the code is
                                    // different
                            ...
                            if (clarifyToExportConcrComp == 1)
                                ...
                                ModelUtils.addIndividualConcreteSubclass2(this.taxonomyExport,
                                        ...
                                        AbstractSuperClass.getLocalName().substring(0,
                                                AbstractSuperClass.getLocalName().length() - 5) + "_CLASS"
                                                + finalversionforLatestAbstractComponent.substring(1,
                                                        finalversionforLatestAbstractComponent.length()));
                                ...
                                Resource source256 = taxonomyExport
                                        .getResource(NEW_TAXONOMY_CLASS.replace("#", "/Component#")
                                                + EncodingUtils.encode(concrComponent.getLocalName().substring(0,
                                                        concrComponent.getLocalName().length() - 5)
                                                        + finalversionforNewAbstractComponent));
                                ...
                                this.dataProps(Constants.COMPONENT_IS_CONCRETE,
                                        concrComponent.getLocalName()
                                                .substring(0, concrComponent.getLocalName().length() - 5).toUpperCase()
                                                + finalversionforNewAbstractComponent,
                                        compConcrete112 + "", XSDDatatype.XSDboolean);
                                ...
                                System.out
                                        .println(
                                                "classisaclass "
                                                        ...
                                                        + "_CLASS" + finalversionforNewAbstractComponent.substring(1,
                                                                finalversionforNewAbstractComponent.length()));
                                System.out.println("secondpara " + AbstractSuperClass.getLocalName()
                                        .substring(0, AbstractSuperClass.getLocalName().length() - 5).toUpperCase()
                                        + finalversionforNewAbstractComponent);
                                this.classIsaClass(AbstractSuperClass.getLocalName()
                                        .substring(0, AbstractSuperClass.getLocalName().length() - 5).toUpperCase()
            ...
            if (typeComp.isURIResource()) { // only adds the type if the type is a uRI (not a blank node)
                ...
                String newchangeforthetype = typeComp.getURI().substring(typeComp.getURI().lastIndexOf("/"),
                        typeComp.getURI().length());
                String newchangeforthetype2 = newchangeforthetype.substring(newchangeforthetype.lastIndexOf("#"),
                        newchangeforthetype.length());
                String tempURI = EncodingUtils.encode(
                        Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + newTemplateName_ + res.getLocalName());
                ...
                OntClass cAux = OPMWModel.createClass(NEW_TAXONOMY_CLASS_2 + "Component" + newchangeforthetype2);// repeated
                                                                                                                 // tuples
                                                                                                                 // will
                                                                                                                 // not
                                                                                                                 // be
            ...
            if (isConcrete != null) {
                ...
                ModelUtils.addDataProperty(OPMWModel,
                        Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + newTemplateName_ + res.getLocalName(),
                        isConcrete.getBoolean() + "", Constants.OPMW_DATA_PROP_IS_CONCRETE, XSDDatatype.XSDboolean);
            }
        ...
        while (r.hasNext()) {
            ...
            String aux = EncodingUtils
                    .encode(Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_ + variable.getLocalName());
            OntClass cAux = OPMWModel.createClass(Constants.OPMW_WORKFLOW_TEMPLATE_ARTIFACT);// repeated tuples will not
                                                                                             // be duplicated
            cAux.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + aux);

            if (dim != null) {// sometimes is null, but it shouldn't
                ModelUtils.addDataProperty(OPMWModel,
                        Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_ + variable.getLocalName(),
                ...
                ModelUtils.addDataProperty(OPMWModel,
                        Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_ + variable.getLocalName(),
                        "" + isCollection, Constants.OPMW_DATA_PROP_IS_COLLECTION, XSDDatatype.XSDboolean);
            }
            ...
            if (type != null) {
                ...
                if (type.isURIResource()) {
                    ...
                    String newchangeforthetype = type.getURI().substring(type.getURI().lastIndexOf("/"),
                            type.getURI().length());
                    String newchangeforthetype2 = newchangeforthetype.substring(newchangeforthetype.lastIndexOf("#"),
                            newchangeforthetype.length());
                    String nameEncoded = EncodingUtils
                            .encode(Constants.CONCEPT_DATA_VARIABLE + "/" + newTemplateName_ + variable.getLocalName());
                    OntClass c = OPMWModel.createClass(NEW_TAXONOMY_CLASS_2 + "Data" + newchangeforthetype2);// repeated
                                                                                                             // tuples
                                                                                                             // will not
                                                                                                             // be
                                                                                                             // duplicated
        ...
        while (r.hasNext()) {
            ...
            // p-plan interop
            OntClass cVar = OPMWModel.createClass(Constants.P_PLAN_Variable);
            cVar.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_PARAMETER_VARIABLE + "/"
                    + EncodingUtils.encode(newTemplateName_ + res.getLocalName()));

            // add the parameter value as an artifact too
            String aux = EncodingUtils
                    .encode(Constants.CONCEPT_PARAMETER_VARIABLE + "/" + newTemplateName_ + res.getLocalName());
            OntClass cAux = OPMWModel.createClass(Constants.OPMW_WORKFLOW_TEMPLATE_ARTIFACT);// repeated tuples will not
                                                                                             // be duplicated
            cAux.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + aux);

            ModelUtils.addProperty(OPMWModel,
                    Constants.CONCEPT_PARAMETER_VARIABLE + "/" + newTemplateName_ + res.getLocalName(),
                    Constants.CONCEPT_WORKFLOW_TEMPLATE + "/" + newTemplateName,
        ...
        while (r.hasNext()) {
            ...
            if (role != null) {
                System.out.println(
                        "Node " + resNode.getLocalName() + " Uses " + resVar.getLocalName() + " Role: " + role);
        ...
        while (r.hasNext()) {
            ...
            if (roleOrig != null && roleDest != null) {
                System.out
                        .println("Artifact " + resVar.getLocalName() + " is generated by node " + resNode.getLocalName()
            ...
            if (roleOrig != null) {
                ...
                ModelUtils.createSubProperty(OPMWModel, Constants.OPMW_PROP_IGB,
                        Constants.PREFIX_EXTENSION + "isGeneratedByAs_" + roleOrig);
                OntProperty propGenerated = OPMWModel
                        .getOntProperty(Constants.PREFIX_EXTENSION + "isGeneratedByAs_" + roleOrig);
                propGenerated.addLabel("Property that indicates that a resource has been generated as a " + roleOrig,
                        "EN");
            }
            if (roleDest != null) {
    ...
    public String transformWINGSResultsToOPMW(String resultFile, String libraryFile, String modeFile,
            String outFilenameOPMW, String outFilenamePROV, String suffix, String data_catalog, String exportMode,
            String dataCatalogDirectory, String domainName, String componentCatalogPath, String componentDirectory) {
        ...
        if (generateExpandedTemplate) {
            hashedExpandedTemplateName = createExpandedTemplate(accname, expandedTemplateName, expandedTemplateURI,
        ...
        if (tool != null) {
            ...
            plan.createIndividual(
                    Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_WORKFLOW_TEMPLATE + "/" + hashedTemplateName);
        ...
        while (r.hasNext()) {
            ...
            Resource blankNode = OPMWModel.createResource(
                    Constants.OPMW_PROP_EXECUTABLE_COMPONENT + "/" + stepName + executableComponentName);
            ...
            String procURI = Constants.PREFIX_EXPORT_RESOURCE + EncodingUtils
                    .encode(Constants.CONCEPT_WORKFLOW_EXECUTION_PROCESS + "/" + stepName + executableComponentName);
        ...
        while (r.hasNext()) {
            ...
            File f = new File(dataCatalogDirectory
                    + inputBinding2.substring(inputBinding2.lastIndexOf("/") + 1, inputBinding2.length()));
            ...
            while (r222.hasNext()) {
                ...
                // metadata
                if (prop.contains("rdf-schema#type") || prop.contains("http://www.w3.org/2000/01/rdf-schema#comment")
            ...
            while (rnew123.hasNext()) {
                ...
                if (res.getLocalName().substring(0, res.getLocalName().indexOf("_")).equals(inputBinding2
                        .substring(inputBinding2.lastIndexOf("/") + 1, inputBinding2.length()).toUpperCase())) {
            ...
            if (similarNamesofInputFiles.isEmpty()) {
                ...
                String nameOfIndividualEnc = EncodingUtils
                        .encode(inputBinding2.substring(inputBinding2.lastIndexOf("/") + 1, inputBinding2.length())
                                .toUpperCase() + "_" + currentMD5 + "_V1");
                ...
                propSelec22 = dataCatalog.createDatatypeProperty(Constants.CATALOG_URI + "DataCatalog#hasMD5");
                Resource orig22 = dataCatalog.getResource(Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT + "/"
                ...
                for (String x : hsprops.keySet()) {
                    ...
                    propSelec23 = dataCatalog.createDatatypeProperty(Constants.CATALOG_URI + "DataCatalog#" + x);
                    Resource orig23 = dataCatalog.getResource(Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT
                            + "/"
                            + EncodingUtils.encode(
                                    inputBinding2.substring(inputBinding2.lastIndexOf("/") + 1, inputBinding2.length())
                                            + "_" + currentMD5 + "_V1"));
                    ...
                    Resource orig2233 = dataCatalog.getResource(Constants.CATALOG_URI + "DataCatalog#" + x);
                ...
                if (!same) {
                    ...
                    System.out.println(
                            "finalversionforLatestAbstractComponent " + finalversionforLatestAbstractComponent);
                    System.out.println("finalversionforNewAbstractComponent " + finalversionforNewAbstractComponent);

                    hmapforInputstoVersions.put(input2,
                            inputBinding2.substring(inputBinding2.lastIndexOf("/") + 1, inputBinding2.length())
                                    .toUpperCase() + "_" + currentMD5 + finalversionforNewAbstractComponent);
                        ...
                        Resource orig23 = dataCatalog
                                .getResource(Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT + "/"
                                        + EncodingUtils.encode(inputBinding2
                                                .substring(inputBinding2.lastIndexOf("/") + 1, inputBinding2.length())
                                                + "_" + currentMD5 + finalversionforNewAbstractComponent));
                        dataCatalog.add(orig23, propSelec23, hsprops.get(x));
                    ...
                    Resource source255 = dataCatalog
                            .getResource(Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT + "/"
                                    + EncodingUtils.encode(inputBinding2.substring(inputBinding2.lastIndexOf("/") + 1,
                                            inputBinding2.length()) + "_" + currentMD5
                                            + finalversionforNewAbstractComponent));
                    Individual instance255 = (Individual) source255.as(Individual.class);
                    if ((compareFile).contains("http://")) {// it is a URI
                        instance255.addProperty(propSelec255,
                                Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT + "/" + compareFile);
                    } else {// it is a local resource
                        instance255.addProperty(propSelec255,
                                dataCatalog.getResource(Constants.OPMW_WORKFLOW_EXECUTION_ARTIFACT_EXPORT_DIRECT + "/"
                                        + EncodingUtils.encode(compareFile)));
                    }
                }
            }
        ...
        while (r.hasNext()) {
            ...
            if (hmapforInputstoVersions.containsKey(var)) {
                auxP = EncodingUtils
                        .encode(Constants.CONCEPT_WORKFLOW_EXECUTION_ARTIFACT + "/" + hmapforInputstoVersions.get(var));
            ...
            // }
                ...
                if (hmapforInputstoVersions.containsKey(var)) {
                    ...
                    propSelec23 = OPMWModel.createDatatypeProperty(Constants.CATALOG_URI + "DataCatalog#" + prop);
                    Resource orig23 = OPMWModel
                            .getResource("http://www.opmw.org/export/resource/WorkflowExecutionArtifact/"
                                    + EncodingUtils.encode(hmapforInputstoVersions.get(var)));
    ...
    private void dataProps(String dataprop, String resourcepart, String propextracted, XSDDatatype x) {
        ...
        Resource orig22 = taxonomyExport
                .getResource(NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#" + EncodingUtils.encode(resourcepart));
        ...
        for (String in : hs) {
            if ((in).contains("http://")) {// it is a URI
                ...
                instance23.addProperty(propSelec23, taxonomyExport
                        .getResource(NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#" + EncodingUtils.encode(in)));
            }
        }
    }
    ...
    public String createExpandedTemplate(String accname, String expandedTemplateName, String expandedTemplateURI,
            String templateName, String domainName, String componentCatalogPath, String componentDirectory) {
        HashMap<String, String> classNames = new HashMap<>();
        NEW_TAXONOMY_CLASS_2 = Constants.CATALOG_URI + domainName + "/";
        // creating a new EXPORT NAME FOR THE TAXONOMY CLASS
        NEW_TAXONOMY_CLASS = Constants.CATALOG_URI + domainName + "#";
        ...
        if (r1.hasNext()) {// there should be just one local name per template
            ...
            ModelUtils.addIndividual(OPMWModel, newExpandedTemplateName, Constants.OPMW_WORKFLOW_EXPANDED_TEMPLATE,
                    "Workflow Expanded Template: " + newExpandedTemplateName);

            // P-PLAN FOR EXPANDED TEMPLATE
            OntClass cParam = OPMWModel.createClass(Constants.P_PLAN_PLAN);
            cParam.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_WORKFLOW_EXPANDED_TEMPLATE
                    + "/" + EncodingUtils.encode(newExpandedTemplateName));

            if (v != null) {
                ModelUtils.addDataProperty(OPMWModel,
                        Constants.CONCEPT_WORKFLOW_EXPANDED_TEMPLATE + "/" + newExpandedTemplateName, "" + v.getInt(),
                        Constants.OPMW_DATA_PROP_VERSION_NUMBER, XSDDatatype.XSDint);
            }
            // add the uri of the original log file (native system template)
            ModelUtils.addDataProperty(OPMWModel,
        ...
        while (r1.hasNext()) {
            ...
            String componentCatalogQueryforActualInputsandOutputsforComponent = Queries
                    .componentCatalogQueryforActualInputsandOutputsforComponent(PREFIX_COMP_CATALOG, tobequeriedclass);
            ...
            while (rformycomponentcatalogabstract.hasNext()) {
                ...
                while (internal1.hasNext()) {
                    ...
                    if (!c.getBoolean()) {
                        if (nodenew.getLocalName().equals(nodenew1.getLocalName()) && nodenew.getLocalName()
                                .toLowerCase().substring(0, nodenew.getLocalName().toLowerCase().lastIndexOf("_"))
                                .equals(res2.getLocalName().substring(0, res2.getLocalName().lastIndexOf("_"))
                                        .toLowerCase())) {
            ...
            while (rformycomponentcatalog.hasNext()) {
                ...
                while (inter.hasNext()) {
                    ...
                    if (c.getBoolean() == true) {
                        if (nodenew.getLocalName().equals(nodenew1.getLocalName()) && nodenew.getLocalName()
                                .toLowerCase().substring(0, nodenew.getLocalName().toLowerCase().lastIndexOf("_"))
                                .equals(res.getLocalName().substring(0, res.getLocalName().length() - 4)
                                        .toLowerCase())) {
                ...
                if (namefoundornot != null && inputsComp.size() == inpsfrommycc.size()
                    ...
                    classNames.put(nodenew.getLocalName().substring(0, nodenew.getLocalName().lastIndexOf("_")),
                            nodenew.getLocalName().substring(0, nodenew.getLocalName().lastIndexOf("_") + 1) + "CLASS"
                                    + nodenew.getLocalName().substring(nodenew.getLocalName().lastIndexOf("_") + 1));
            ...
            if (flag == 1) {
                ...
                try {
                    ...
                    this.dataProps(Constants.COMPONENT_HAS_MD5_CODE,
                            res.getLocalName().substring(0, res.getLocalName().length() - 4).toUpperCase() + "_V1",
                            EncodingUtils.MD5ComponentCode(componentDirectory
                                    + compLoc.substring(compLoc.lastIndexOf("/") + 1, compLoc.length()) + ".zip"),
                            XSDDatatype.XSDstring);
                } catch (Exception e) {
                ...
                ModelUtils.addIndividualConcreteSubclass2(this.taxonomyExport,
                        ...
                        finalabstractcomponenttobelinkedto.substring(0,
                                ...
                                + finalabstractcomponenttobelinkedto.substring(
                                        finalabstractcomponenttobelinkedto.lastIndexOf("_") + 1,
                                        finalabstractcomponenttobelinkedto.length()));
                ...
                this.dataProps(Constants.RDFS_LABEL,
                        res.getLocalName().substring(0, res.getLocalName().length() - 4).toUpperCase() + "_V1",
                        res.getLocalName().substring(0, res.getLocalName().length() - 4), XSDDatatype.XSDstring);
                ...
                this.dataProps(Constants.COMPONENT_HAS_LOCATION,
                        res.getLocalName().substring(0, res.getLocalName().length() - 4).toUpperCase() + "_V1",
                        componentDirectory + compLoc.substring(compLoc.lastIndexOf("/") + 1, compLoc.length()),
                        XSDDatatype.XSDstring);
                ...
                try {
                    ...
                    this.dataProps(Constants.COMPONENT_HAS_MD5_CODE,
                            res.getLocalName().substring(0, res.getLocalName().length() - 4).toUpperCase() + "_V"
                                    + versionnumber,
                            EncodingUtils.MD5ComponentCode(componentDirectory
                                    + compLoc.substring(compLoc.lastIndexOf("/") + 1, compLoc.length()) + ".zip"),
                            XSDDatatype.XSDstring);
                ...
                ModelUtils.addIndividualConcreteSubclass2(this.taxonomyExport,
                        res.getLocalName().substring(0, res.getLocalName().length() - 4).toUpperCase() + "_CLASSV"
                                + versionnumber,
                        finalabstractcomponenttobelinkedto.substring(0,
                                finalabstractcomponenttobelinkedto.lastIndexOf("_") + 1)
                                + "CLASS"
                ...
                Resource source255 = taxonomyExport.getResource(NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#"
                        + EncodingUtils
                                .encode(res.getLocalName().substring(0, res.getLocalName().length() - 4).toUpperCase()
                                        + "_V" + versionnumber));
                Individual instance255 = (Individual) source255.as(Individual.class);
                if ((res.getLocalName().toUpperCase() + "_V" + max).contains("http://")) {// it is a URI
                    instance255.addProperty(propSelec255,
                            NEW_TAXONOMY_CLASS.replace("#", "") + "/Component#"
                                    + res.getLocalName().substring(0, res.getLocalName().length() - 4).toUpperCase()
                                    + "_V" + max);
                } else {// it is a local resource
                    instance255.addProperty(propSelec255, taxonomyExport.getResource(NEW_TAXONOMY_CLASS.replace("#", "")
                            + "/Component#"
                            + EncodingUtils.encode(
                                    res.getLocalName().substring(0, res.getLocalName().length() - 4).toUpperCase()
                                            + "_V" + max)));
            ...
            ModelUtils.addIndividual(OPMWModel, "Expanded_" + newExpandedTemplateName + "_" + res.getLocalName(),
            ...
            OntClass cStep = OPMWModel.createClass(Constants.P_PLAN_STEP);
            cStep.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/"
                    + EncodingUtils.encode("Expanded_" + newExpandedTemplateName + "_" + res.getLocalName()));
            ...
            if (typeComp.isURIResource()) { // only adds the type if the type is a uRI (not a blank node)
                ...
                String newchangeforthetype2 = newchangeforthetype.substring(newchangeforthetype.lastIndexOf("#"),
                        newchangeforthetype.length());
                String tempURI = EncodingUtils.encode(Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + "Expanded_"
                ...
                OntClass cAux1 = OPMWModel.createClass(NEW_TAXONOMY_CLASS_2 + "Component" + newchangeforthetype2);// repeated
                                                                                                                  // tuples
                                                                                                                  // will
                                                                                                                  // not
                                                                                                                  // be
            ...
            if (rule != null) {
                // rules are strings
                ModelUtils.addDataProperty(OPMWModel,
                        Constants.CONCEPT_WORKFLOW_TEMPLATE_PROCESS + "/" + "Expanded_" + newExpandedTemplateName + "_"
                                + res.getLocalName(),
                        rule.getString(),
                        Constants.WINGS_DATA_PROP_HAS_RULE);
            }
        ...
        while (r1.hasNext()) {
            ...
            ModelUtils.addIndividual(OPMWModel, "Expanded_" + newExpandedTemplateName + "_" + variable.getLocalName(),
                    Constants.OPMW_DATA_VARIABLE, "Data variable " + variable.getLocalName());

            // p-plan interop
            OntClass cVar = OPMWModel.createClass(Constants.P_PLAN_Variable);
            cVar.createIndividual(Constants.PREFIX_EXPORT_RESOURCE + Constants.CONCEPT_DATA_VARIABLE + "/"
                    + EncodingUtils.encode("Expanded_" + newExpandedTemplateName + "_" + variable.getLocalName()));
            ...
            OntClass cAux1 = OPMWModel.createClass(Constants.OPMW_WORKFLOW_TEMPLATE_ARTIFACT);// repeated tuples will
                                                                                              // not be duplicated
            ...
            ModelUtils.addProperty(OPMWModel,
                    Constants.CONCEPT_DATA_VARIABLE + "/" + "Expanded_" + newExpandedTemplateName + "_"
        ...
        while (r1.hasNext()) {
            ...
            ModelUtils.addProperty(OPMWModel,
                    Constants.CONCEPT_PARAMETER_VARIABLE + "/" + "Expanded_" + newExpandedTemplateName + "_"
                            + res.getLocalName(),
                    Constants.CONCEPT_WORKFLOW_EXPANDED_TEMPLATE + "/" + newExpandedTemplateName,
                    Constants.OPMW_PROP_IS_PARAMETER_OF_TEMPLATE);
            ...
            ModelUtils.addProperty(OPMWModel,
                    Constants.CONCEPT_PARAMETER_VARIABLE + "/" + "Expanded_" + newExpandedTemplateName + "_"
                            + res.getLocalName(),
                    Constants.CONCEPT_PARAMETER_VARIABLE + "/" + templateName + "/" + derivedFrom.getLocalName(),
            ...
            ModelUtils.addDataProperty(OPMWModel,
                    Constants.CONCEPT_PARAMETER_VARIABLE + "/" + "Expanded_" + newExpandedTemplateName + "_"
                            + res.getLocalName(),
                    parValue + "", Constants.OPMW_PROP_IS_PARVALUE_OF_EXPANDED_TEMPLATE_PARAMETER_VARIABLE);
    ...
    private void exportHardwareSoftwareDependencies(boolean needsval, Float memval, Float stval, Float minversionval,
            Resource concrComponent, int version_no) {
        Resource blankNode112 = taxonomyExport.createResource(Constants.PREFIX_RESOURCE + "HardwareRequirements_"
                + concrComponent.getLocalName().substring(0, concrComponent.getLocalName().length() - 5) + "_V"
                + version_no);
        ...
        if (memval != null) {
            blankNode112.addProperty(taxonomyExport.createOntProperty(Constants.WINGS_PROP_REQUIRES_MEMORYGB),
        ...
        String procURI = NEW_TAXONOMY_CLASS.replace("#", "/Component#") + EncodingUtils
                .encode(concrComponent.getLocalName().substring(0, concrComponent.getLocalName().length() - 5) + "_V"
                        + version_no);
        ...
        this.classIsaClassHardwareParts(Constants.HARDWARE_DEPENDENCY,
                Constants.PREFIX_RESOURCE + "HardwareRequirements_"
                        + concrComponent.getLocalName().substring(0, concrComponent.getLocalName().length() - 5) + "_V"
                        + version_no);
        Resource blankNode113 = taxonomyExport.createResource(Constants.PREFIX_RESOURCE + "SoftwareRequirements_"
                + concrComponent.getLocalName().substring(0, concrComponent.getLocalName().length() - 5) + "_V"
                + version_no);
